...
List<String> _natChunks(String s) => RegExp(r'\d+|\D+')
...
class _DevicesScreenState extends State<DevicesScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance
        .addPostFrameCallback((_) => context.read<AppProvider>().loadDevices());
  }

  @override
  Widget build(BuildContext context) {
    final provider = context.watch<AppProvider>();
    final q = (_localSearch.isNotEmpty ? _localSearch : widget.searchQuery)
        .toLowerCase();
    ...
    return Column(
      children: [
        _FilterTabs(
          tabs: [
            _Tab('all', tr('filter_all')),
            _Tab('on', tr('filter_online')),
            _Tab('moving', tr('dash_moving')),
            _Tab('off', tr('filter_offline'))
          ],
          ...
          counts: {
            ...
            'off': provider.tabDevices
                .where((d) => d.isOffline || d.isInactive)
                .length,
          },
        ),
        Padding(
...
class _DeviceRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final dotColor = device.isMoving
...
class _ClientsScreenState extends State<ClientsScreen> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance
        .addPostFrameCallback((_) => context.read<AppProvider>().loadUsers());
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (filter == 'all' || filter == 'dealer') {
      if (user?.isAdmin == true) {
        ...
        for (final dealer in dealers) {
          ...
          if (filter == 'all') {
            final subs =
                subDealers.where((s) => s.dealerId == dealer.id).toList();
            final subDealerIds = subs.map((s) => s.id).toSet();
            for (final sub in subs) {
              items.add(_HierarchyItem(user: sub, level: 1));
              final subClients =
                  clients.where((c) => c.parentId == sub.id).toList();
              for (final cl in subClients) {
                items.add(_HierarchyItem(user: cl, level: 2));
              }
            }
            final directClients = clients
                .where((c) =>
                    c.dealerId == dealer.id &&
                    (c.parentId == null ||
                        c.parentId == dealer.id ||
                        !subDealerIds.contains(c.parentId)))
                .toList();
            for (final cl in directClients) {
              items.add(_HierarchyItem(user: cl, level: 1));
            }
          }
        }
        ...
        if (filter == 'all') {
          ...
          for (final u in [...subDealers, ...clients]) {
            if (!addedIds.contains(u.id))
              items.add(_HierarchyItem(user: u, level: 0));
          }
        }
      } else if (filter == 'all') {
        ...
        final directClients = clients
            .where(
                (c) => c.parentId == null || !subDealerIds.contains(c.parentId))
            .toList();
        for (final cl in directClients) {
    ...
    final tabs = user?.isAdmin == true
        ? [
            _Tab('all', tr('filter_all')),
            _Tab('dealer', tr('tab_dealer')),
            _Tab('sub', tr('tab_subdealer')),
            _Tab('client', tr('tab_client'))
          ]
        : [
            _Tab('all', tr('filter_all')),
            _Tab('sub', tr('tab_subdealer')),
    ...
    return Column(
      children: [
        _FilterTabs(
            tabs: tabs,
            current: filter,
            onTap: provider.setClientFilter,
            counts: counts),
        Padding(
            padding: const EdgeInsets.fromLTRB(12, 10, 12, 4),
            child: _SearchField(
                hint: tr('search_client'),
                onChanged: (v) => setState(() => _localSearch = v))),
        Expanded(
...
class _UserProfileSheetState extends State<_UserProfileSheet>
  ...
  String?
      _avatarOverride; // الصورة الجديدة بعد الرفع (الموديل المخزّن بيفضل على الرابط القديم)
  ...
  Future<void> _loadDevices() async {
    if (_devices.isEmpty) {
      final providerDevices = context
          .read<AppProvider>()
          .devices
          .where((d) => d.userId == widget.user.id)
          .toList();
    ...
    try {
      ...
      if (raw is List) {
        final list = raw
            ...
            .toList();
        list.sort(
            (a, b) => naturalCompare(a.name, b.name)); // ترتيب طبيعي بالاسم
        _devices = list;
      }
  ...
  List<DeviceModel> get _statDevices {
    ...
    final did = u.id;
    final ids = <int>{did};
    for (final x in provider.users) {
      if (x.dealerId == did || x.parentId == did) ids.add(x.id);
    }
    for (final x in provider.users) {
      if (x.parentId != null && ids.contains(x.parentId)) ids.add(x.id);
    }
    ...
    return list.isNotEmpty
        ? list
        : _devices; // fallback لو داتا الشجرة مش متاحة (viewer مش أدمن)
  }
  ...
  Future<void> _loadSubUsers() async {
    ...
    try {
      ...
      if (widget.user.isSubDealer) {
        ...
        _subUsers =
            allUsers.where((u) => u.parentId == widget.user.id).toList();
      } else {
        // الديلر: كل المستخدمين اللي dealer_id بتاعهم = id الديلر
        _subUsers =
            allUsers.where((u) => u.dealerId == widget.user.id).toList();
      }
    } catch (_) {
      _subUsers = [];
    }
  ...
  Future<void> _loadCardBalance() async {
    try {
      ...
      if (result['success'] == true && result['balance'] != null) {
        ...
        if (mounted) {
          setState(() => _cardBalance = {
                'new_yearly':
                    int.tryParse(b['new_yearly']?.toString() ?? '0') ?? 0,
                'new_lifetime':
                    int.tryParse(b['new_lifetime']?.toString() ?? '0') ?? 0,
                'renew_yearly':
                    int.tryParse(b['renew_yearly']?.toString() ?? '0') ?? 0,
                'renew_lifetime':
                    int.tryParse(b['renew_lifetime']?.toString() ?? '0') ?? 0,
              });
        }
      }
    } catch (_) {}
  ...
  Widget build(BuildContext context) {
    final u = widget.user;
    final badgeColor = u.isDealer
        ? const Color(0xFFC41E3A)
        : u.isSubDealer
            ? const Color(0xFF7C3AED)
            : const Color(0xFF1565C0);
    final badgeLabel = u.isDealer
        ? tr('tab_dealer')
        : u.isSubDealer
            ? tr('tab_subdealer')
            : tr('tab_client');
          ...
          children: [
            ...
            Container(
              ...
              padding: EdgeInsets.fromLTRB(
                  12, MediaQuery.of(context).padding.top + 6, 12, 0),
              child: Container(
                  ...
                  children: [
                    ...
                    Row(children: [
                      _HeaderStatTile(
                          ...
                          label: tr('cl_devices'),
                          value: (u.isDealer || u.isSubDealer)
                              ? _statDevices.length
                              : (_loadingDevices
                                  ? u.deviceCount
                                  : _devices.length),
                          color: const Color(0xFF6BA539),
                          bg: const Color(0x336BA539)),
                      ...
                      if (_statDevices.isNotEmpty) ...[
                        ...
                        _HeaderStatTile(
                            ...
                            label: tr('cl_offline'),
                            value:
                                _statDevices.where((d) => d.isOffline).length,
                            color: const Color(0xFFEF5350),
                            bg: const Color(0x33EF5350)),
                      ],
                    ]),
                    if ((u.isDealer || u.isSubDealer) &&
                      ...
                      Row(children: [
                        _ProfileBalTile(
                            ...
                            label: tr('cl_new_yearly'),
                            value: _cardBalance!['new_yearly'] ?? 0,
                            color: const Color(0xFF2196F3),
                            bg: const Color(0xFFE3F2FD)),
                        const SizedBox(width: 6),
                        _ProfileBalTile(
                            icon: Icons.all_inclusive_outlined,
                            label: tr('cl_new_life'),
                            value: _cardBalance!['new_lifetime'] ?? 0,
                            color: const Color(0xFF7C3AED),
                            bg: const Color(0xFFF3E8FF)),
                        const SizedBox(width: 6),
                        _ProfileBalTile(
                            icon: Icons.autorenew_outlined,
                            label: tr('cl_renew_yearly'),
                            value: _cardBalance!['renew_yearly'] ?? 0,
                            color: const Color(0xFF059669),
                            bg: const Color(0xFFDCFCE7)),
                        const SizedBox(width: 6),
                        _ProfileBalTile(
                            icon: Icons.loop_outlined,
                            label: tr('cl_renew_life'),
                            value: _cardBalance!['renew_lifetime'] ?? 0,
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  _ActionBtn(
                      ...
                      label: tr('cl_act_edit'),
                      color: const Color(0xFF1565C0),
                      onTap: () => _editUser(context)),
                  _ActionBtn(
                      icon: Icons.add_to_queue_outlined,
                      label: tr('cl_act_device'),
                      color: const Color(0xFF6BA539),
                      onTap: () => _addDevice(context)),
                  _ActionBtn(
                      icon: Icons.lock_reset_outlined,
                      label: tr('cl_act_password'),
                      color: const Color(0xFFF59E0B),
                      onTap: () => _resetPassword(context)),
                  _ActionBtn(
                      icon: Icons.settings_outlined,
                      label: tr('cl_act_settings'),
                  ...
                  _ActionBtn(
                      ...
                      onTap: () => openAutoEngine(context, widget.user.id,
                          subtitle: widget.user.fullName)),
                  _ActionBtn(
                      icon: Icons.swap_horiz_outlined,
                      label: tr('cl_act_transfer'),
                      color: const Color(0xFF7C3AED),
                      onTap: () => _transferAccount(context)),
                  _ActionBtn(
                      icon: Icons.delete_outline,
                      label: tr('cl_act_delete'),
                      color: const Color(0xFFEF5350),
                  ...
                  if (context.read<AppProvider>().currentUser?.accountType ==
                      'admin')
                    _ActionBtn(
                        icon: Icons.vpn_key_outlined,
                        label: tr('cl_act_apikey'),
                        color: const Color(0xFF475569),
                        onTap: () => _apiKeyDialog(context)),
                      ...
                      : _devices.isEmpty
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (_, i) => _DeviceProfileRow(
                                ...
                                onDelete: () =>
                                    _deleteDevice(context, _devices[i]),
                                onTransfer: () =>
                                    _transferDevice(context, _devices[i]),
                              ),
                            ),
                  ...
                  SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                        _InfoCard(items: [
                          _InfoItem(
                              label: tr('cl_username'),
                              value: u.username,
                              copyable: true),
                          _InfoItem(
                              label: tr('cl_acc_type'), value: u.accountTypeAr),
                          _InfoItem(
                              label: tr('cl_phone'), value: u.phone ?? '-'),
                          _InfoItem(
                              label: tr('cl_mobile'), value: u.mobile ?? '-'),
                          _InfoItem(
                              label: tr('cl_email'), value: u.email ?? '-'),
                          _InfoItem(
                              label: tr('cl_address'), value: u.address ?? '-'),
                          _InfoItem(
                              label: tr('cl_created'),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => SafeArea(
        ...
        if ((_avatarOverride ?? u.avatarUrl) != null)
          ListTile(
              leading:
                  const Icon(Icons.image_outlined, color: Color(0xFF1565C0)),
              title: Text(tr('cl_view_photo'),
                    ...
                    builder: (d) => Dialog(
                        ...
                        child: GestureDetector(
                            ...
                            child: ClipRRect(
                                ...
                                child: Image.network(
                                    (_avatarOverride ?? u.avatarUrl)!)))));
              }),
        ListTile(
            leading:
                const Icon(Icons.camera_alt_outlined, color: Color(0xFFC41E3A)),
            title: Text(tr('cl_change_photo'),
                style: const TextStyle(fontFamily: 'Cairo')),
            onTap: () async {
              Navigator.pop(ctx);
              ...
              try {
                ...
                final r = await ApiService.request('upload_avatar', {
                  'image': base64Encode(bytes),
                  'ext': image.path.split('.').last.toLowerCase(),
                  'userId': u.id
                });
  ...
  void _openMessages(BuildContext context) {
    ...
    Future<void> loadMsgs(StateSetter setS, [int attempt = 0]) async {
      ...
      try {
        ...
        for (final m in all) {
          ...
          if (!((sid == uid && rid == me) || (sid == me && rid == uid)))
            continue;
          final k = (m['id'] ??
                  '${m['sender_id']}_${m['receiver_id']}_${m['created_at']}')
              .toString();
          if (seen.add(k)) conv.add(m);
        }
        conv.sort((a, b) => (a['created_at'] ?? '')
            .toString()
            .compareTo((b['created_at'] ?? '').toString()));
        setS(() {
          msgs = conv.reversed.toList();
          loading = false;
        });
      } catch (_) {
        if (attempt < 2) {
          await Future.delayed(const Duration(milliseconds: 600));
          return loadMsgs(setS, attempt + 1);
        }
        setS(() => loading = false);
      }
    }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) {
        if (!started) {
          started = true;
          WidgetsBinding.instance.addPostFrameCallback((_) => loadMsgs(setS));
        }
        return DraggableScrollableSheet(
          ...
          builder: (_, sc) => Container(
            ...
            child: Column(children: [
              ...
              Padding(
                  ...
                  child: Row(children: [
                    _UserAvatar(
                        ...
                        radius: 16,
                        bgColor: const Color(0xFFC41E3A)),
                    const SizedBox(width: 10),
                    Expanded(
                        child: Text(
                            tr('cl_msgs_title', {'name': widget.user.fullName}),
                            style: const TextStyle(
                                fontFamily: 'Cairo',
                                fontSize: 14,
                                fontWeight: FontWeight.w600))),
              ...
              Padding(
                padding: EdgeInsets.fromLTRB(
                    12, 8, 12, 12 + MediaQuery.of(ctx).viewInsets.bottom),
                child: Row(children: [
                  Expanded(
                      child: TextField(
                    controller: sendCtrl,
            ...
            child: Column(
              ...
              children: [
                Row(
                    ...
                    children: [
                      Text(tr('cl_edit_title', {'name': widget.user.fullName}),
                          style: const TextStyle(
                              fontSize: 14,
                              fontWeight: FontWeight.w600,
                              fontFamily: 'Cairo')),
                ...
                TextField(
                    ...
                    textDirection:
                        I18n.isAr ? TextDirection.rtl : TextDirection.ltr,
                    decoration: InputDecoration(labelText: tr('cl_full_name'))),
                const SizedBox(height: 10),
                TextField(
                    controller: phoneCtrl,
                    decoration: InputDecoration(labelText: tr('cl_phone')),
                    keyboardType: TextInputType.phone),
                const SizedBox(height: 10),
                TextField(
                    controller: mobileCtrl,
                    decoration: InputDecoration(labelText: tr('cl_mobile')),
                    keyboardType: TextInputType.phone),
                const SizedBox(height: 10),
                TextField(
                    controller: emailCtrl,
                    decoration: InputDecoration(labelText: tr('cl_email')),
                ...
                TextField(
                    ...
                    textDirection:
                        I18n.isAr ? TextDirection.rtl : TextDirection.ltr,
                    decoration: InputDecoration(labelText: tr('cl_address'))),
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) {
        Future<void> call(String action) async {
          setS(() => busy = true);
          final r =
              await ApiService.request(action, {'user_id': widget.user.id});
        ...
        return AlertDialog(
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content: Text(tr('cl_apikey_copied'),
                                      style: cairo)),
                            );
                          },
                        ),
          ...
          actions: [
            ...
            TextButton(
                onPressed: () => Navigator.pop(ctx),
                child: Text(tr('close'), style: cairo)),
          ],
        );
      }),
    );
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) {
        Future<void> save() async {
          ...
          if (r['success'] == true) {
            ...
            setS(() {
              busy = false;
              err = r['error']?.toString() ?? tr('dash_failed');
            });
          }
        }
        ...
        return Padding(
          ...
          child: Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Expanded(
                      child: Wrap(
                          ...
                          children: types
                              .map((t) => ChoiceChip(
                                    label: Text(label(t),
                                        style: cairo.copyWith(fontSize: 12)),
                                    selected: type == t,
                                    onSelected: busy
                                        ? null
                                        : (_) => setS(() => type = t),
              ...
              TextField(
                  controller: userCtrl,
                  decoration: InputDecoration(labelText: tr('dash_username'))),
              const SizedBox(height: 8),
              TextField(
                  controller: nameCtrl,
                  decoration: InputDecoration(labelText: tr('dash_full_name'))),
              const SizedBox(height: 8),
              TextField(
                  controller: phoneCtrl,
                  keyboardType: TextInputType.phone,
              ...
              TextField(
                  ...
                  obscureText: true,
                  decoration:
                      InputDecoration(labelText: tr('dash_confirm_password'))),
              if (err != null) ...[
                const SizedBox(height: 10),
                Text(err!,
                    style: cairo.copyWith(
                        color: const Color(0xFFEF5350), fontSize: 12)),
              ],
              const SizedBox(height: 16),
              SizedBox(
                  width: double.infinity,
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                ImeiField(
                    controller: imeiCtrl,
                    decoration: InputDecoration(labelText: tr('cl_imei'))),
                const SizedBox(height: 10),
                TextField(
                    controller: nameCtrl,
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: [
                    DropdownMenuItem(
                        value: 'lifetime', child: Text(tr('cl_lifetime'))),
                    DropdownMenuItem(
                        value: 'annual', child: Text(tr('cl_annual'))),
                  ],
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: saving
                        ? null
                        : () async {
                            if (imeiCtrl.text.isEmpty || nameCtrl.text.isEmpty)
                              return;
                            setS(() => saving = true);
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) {
        Future<void> apply({String? pw}) async {
          ...
          final r = await ApiService.resetPassword(
              userId: widget.user.id, newPassword: pw);
          if (!ctx.mounted) return;
          if (r['success'] == true) {
            Navigator.pop(ctx);
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                  content: Text(
                      pw == null ? tr('cl_pw_reset') : tr('cl_pw_changed'),
                      style: cairo),
            ...
            setS(() {
              busy = false;
              err = r['error']?.toString() ?? tr('api_error');
            });
          }
        }
          ...
          content: customMode
              ...
              ? Column(mainAxisSize: MainAxisSize.min, children: [
                  TextField(
                      ...
                      style: cairo,
                      decoration: InputDecoration(
                          labelText: tr('cl_pw_new'), labelStyle: cairo)),
                  const SizedBox(height: 8),
                  TextField(
                      ...
                      style: cairo,
                      decoration: InputDecoration(
                          labelText: tr('cl_pw_confirm'), labelStyle: cairo)),
                  if (err != null)
                    Padding(
              ...
              : Column(mainAxisSize: MainAxisSize.min, children: [
                  ListTile(
                    ...
                    leading:
                        const Icon(Icons.lock_reset, color: Color(0xFFF59E0B)),
                    title: Text(tr('cl_pw_reset_default'),
                        style: const TextStyle(
                            fontFamily: 'Cairo',
                            fontSize: 13,
                            fontWeight: FontWeight.w600)),
                  ...
                  if (busy)
                    const Padding(
                        ...
                        child: SizedBox(
                            ...
                            height: 20,
                            child: CircularProgressIndicator(
                                strokeWidth: 2, color: Color(0xFFC41E3A)))),
                ]),
          actions: customMode
              ? [
                  TextButton(
                  ...
                  ElevatedButton(
                    onPressed: busy
                        ...
                        : () {
                            ...
                            if (p.length < 6) {
                              setS(() => err = tr('cl_pw_too_short'));
                              return;
                            }
                            if (p != confirmCtrl.text.trim()) {
                              setS(() => err = tr('cl_pw_mismatch'));
                              return;
                            }
                            apply(pw: p);
                          },
              ...
              : [
                  TextButton(
                      onPressed: () => Navigator.pop(ctx),
                      child: Text(tr('cancel'), style: cairo))
                ],
        );
      }),
    );
  ...
  void _openSettings(BuildContext context) async {
    ...
    final result =
        await ApiService.request('get_settings', {'id': widget.user.id});
    ...
    final callerRole =
        context.read<AppProvider>().currentUser?.accountType ?? '';
    final canChangeRole = callerRole == 'dealer' ||
        callerRole == 'sub_dealer' ||
        callerRole == 'admin';

    final tzCtrl =
        TextEditingController(text: _settings['timezone'] ?? 'Africa/Cairo');
    String selectedRole = widget.user.accountType; // current account type
    bool viewOnly =
        (_settings['view_only'] == true || _settings['view_only'] == 1);
    bool saving = false;
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      InkWell(
                        onTap: () {
                          Clipboard.setData(
                              ClipboardData(text: widget.user.username));
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            content: Text(tr('imei_copied'),
                                style: const TextStyle(fontFamily: 'Cairo')),
                            duration: const Duration(seconds: 1),
                            backgroundColor: const Color(0xFF6BA539),
                          ));
                  ...
                  if (canChangeRole) ...[
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: [
                        DropdownMenuItem(
                            value: 'client',
                            child: Text(tr('tab_client'),
                                style: const TextStyle(fontFamily: 'Cairo'))),
                        if (callerRole == 'admin' || callerRole == 'dealer')
                          DropdownMenuItem(
                              value: 'sub_dealer',
                              child: Text(tr('tab_subdealer'),
                                  style: const TextStyle(fontFamily: 'Cairo'))),
                        if (callerRole == 'admin')
                          DropdownMenuItem(
                              value: 'dealer',
                              child: Text(tr('tab_dealer'),
                                  style: const TextStyle(fontFamily: 'Cairo'))),
                      ],
                    ...
                    child: Row(children: [
                      Expanded(
                          child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                            Text(tr('cl_view_only'),
                                style: const TextStyle(
                                    fontFamily: 'Cairo', fontSize: 13)),
                            Text(tr('cl_view_only_hint'),
                                style: const TextStyle(
                                    fontFamily: 'Cairo',
                                    fontSize: 11,
                                    color: Color(0xFF6B7280))),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: [
                      DropdownMenuItem(
                          value: 'Africa/Cairo',
                          child: Text(tr('cl_tz_cairo'),
                              style: const TextStyle(fontFamily: 'Cairo'))),
                      DropdownMenuItem(
                          value: 'Asia/Riyadh',
                          child: Text(tr('cl_tz_riyadh'),
                              style: const TextStyle(fontFamily: 'Cairo'))),
                      DropdownMenuItem(
                          value: 'Asia/Dubai',
                          child: Text(tr('cl_tz_dubai'),
                              style: const TextStyle(fontFamily: 'Cairo'))),
                      DropdownMenuItem(
                          value: 'Europe/London',
                          child: Text(tr('cl_tz_london'),
                              style: const TextStyle(fontFamily: 'Cairo'))),
                    ],
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: saving
                          ...
                          : () async {
                              ...
                              // Save settings
                              final r =
                                  await ApiService.request('save_settings', {
                                'id': widget.user.id,
                                'settings': _settings,
                              });
                              ...
                              String? roleErr;
                              if (canChangeRole &&
                                  selectedRole != widget.user.accountType) {
                                final rr = await ApiService.updateUser(
                                  userId: widget.user.id,
                                  updates: {'account_type': selectedRole},
                                );
                                // فشل تغيير الدور كان يمرّ صامتًا فتظهر «تم الحفظ» والدور كما هو
                                if (rr['success'] != true)
                                  roleErr =
                                      rr['error'] as String? ?? tr('cl_fail');
                              }
                              ...
                              if (ctx.mounted) {
                                ...
                                ScaffoldMessenger.of(context)
                                    .showSnackBar(SnackBar(
                                  content: Text(
                                      (r['success'] == true && roleErr == null)
                                          ? tr('cl_settings_saved')
                                          : (roleErr ??
                                              r['error'] ??
                                              tr('cl_fail')),
                                      style:
          ...
          child: Column(
            ...
            children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text(tr('cl_transfer_title', {'name': widget.user.fullName}),
                    style: const TextStyle(
                        fontSize: 14,
                        fontWeight: FontWeight.w600,
                        fontFamily: 'Cairo')),
                ...
                child: ElevatedButton(
                  ...
                  onPressed: saving
                      ...
                      : () async {
                          ...
                          final r =
                              await ApiService.request('transfer_account', {
                            'userId': widget.user.id,
  ...
  void _deleteAccount(BuildContext context) async {
    ...
    if (_devices.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(tr('cl_del_acc_has_devices'),
            style: const TextStyle(fontFamily: 'Cairo')),
        backgroundColor: const Color(0xFFC41E3A),
      ));
      return;
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(tr('cl_del_acc_title'),
            style: const TextStyle(fontFamily: 'Cairo')),
        content: Text(tr('cl_del_acc_q', {'name': widget.user.fullName}),
            style: const TextStyle(fontFamily: 'Cairo')),
        actions: [
          TextButton(
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(
          ok ? tr('cl_acc_deleted') : (r['error']?.toString() ?? tr('cl_fail')),
          style: const TextStyle(fontFamily: 'Cairo')),
  ...
  void _editDevice(BuildContext context, DeviceModel device) {
    showModalBottomSheet(
        ...
        backgroundColor: Colors.transparent,
        builder: (_) =>
            _EditDeviceSheet(device: device, onSaved: _loadDevices));
  }
  ...
  void _deleteDevice(BuildContext context, DeviceModel device) async {
    ...
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(tr('cl_del_dev_title'),
            style: const TextStyle(fontFamily: 'Cairo')),
        content: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(tr('cl_del_dev_q', {'name': device.name}),
                  style: const TextStyle(fontFamily: 'Cairo')),
              const SizedBox(height: 12),
              TextField(
                  controller: pwCtrl,
                  obscureText: true,
                  decoration: InputDecoration(
                      labelText: I18n.isAr ? 'كلمة المرور' : 'Password',
                      labelStyle:
                          const TextStyle(fontFamily: 'Cairo', fontSize: 12),
                      border: const OutlineInputBorder(),
                      isDense: true)),
    ...
    if (confirm == true && mounted) {
      final r = await ApiService.request(
          'delete_device', {'id': device.id, 'password': pwCtrl.text});
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
              r['success'] == true
            ...
            setS(() {
              filtered = q.isEmpty
                  ...
                  : targets
                      ...
                      .toList();
              if (selectedId != null &&
                  !filtered.any((u) => u.id == selectedId)) selectedId = null;
            });
          }
            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  Expanded(
                    child: TextField(
                      ...
                      onChanged: (v) {
                        onSearch(v);
                        if (v.isNotEmpty) setS(() => myAccount = false);
                      },
                  ...
                  GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color:
                                myAccount ? const Color(0xFFC41E3A) : divColor),
                      ),
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        itemBuilder: (_, i) {
                          ...
                          final badgeColor = u.isDealer
                              ...
                              : u.isSubDealer
                                  ...
                                  : const Color(0xFF6BA539);
                          final badgeLabel = u.isDealer
                              ? (I18n.isAr ? 'ديلر' : 'Dealer')
                              : u.isSubDealer
                                  ? (I18n.isAr ? 'موزع' : 'Sub')
                                  : (I18n.isAr ? 'عميل' : 'Client');
                          return ListTile(
                            dense: true,
                            selected: isSelected,
                            selectedTileColor:
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    style: ElevatedButton.styleFrom(
                      backgroundColor: (myAccount || selectedId != null)
                          ? const Color(0xFF1565C0)
                          : Colors.grey,
                      padding: const EdgeInsets.symmetric(vertical: 13),
                    ),
                    onPressed: (saving || (!myAccount && selectedId == null))
                        ? null
                        : () async {
                            setS(() => saving = true);
                            final targetId = myAccount ? me?.id : selectedId;
                            final r =
                                await ApiService.request('transfer_device', {
                              'imei': device.imei,
                              'userId': targetId,
                            });
                            setS(() => saving = false);
                ...
                Row(children: [
                  ...
                  Expanded(
                      child: TextField(
                          controller: nlCtrl,
                          decoration: InputDecoration(
                              labelText: tr('cl_new_life_cur', {'n': '$oldNl'}),
                              border: const OutlineInputBorder()),
                          keyboardType: TextInputType.number)),
                ]),
                const SizedBox(height: 8),
                Row(children: [
                  Expanded(
                      child: TextField(
                          ...
                          decoration: InputDecoration(
                              labelText:
                                  tr('cl_renew_yearly_cur', {'n': '$oldRy'}),
                              border: const OutlineInputBorder()),
                          keyboardType: TextInputType.number)),
                  const SizedBox(width: 8),
                  Expanded(
                    ...
                    onPressed: saving
                        ...
                        : () async {
                            ...
                            final assigns = <String, dynamic>{
                              'dealer_id': widget.user.id
                            };
                            final deducts = <String, dynamic>{
                              'dealer_id': widget.user.id
                            };
...
class _ClientMapViewState extends State<_ClientMapView> {
  ...
  void _fitBounds() {
    final pts =
        widget.devices.where((d) => d.lat != null && d.lng != null).toList();
    if (pts.isEmpty || _ctrl == null) return;
    if (pts.length == 1) {
      _ctrl!.animateCamera(CameraUpdate.newLatLngZoom(
          LatLng(pts.first.lat!, pts.first.lng!), 14));
      return;
    }
...
String _timeAgo(DateTime? dt) {
  ...
  if (diff.inSeconds < 60)
    return ' ${tr('cl_ago_sec', {
          'n': '${diff.inSeconds < 1 ? 1 : diff.inSeconds}'
        })}';
  if (diff.inMinutes < 60)
    return ' ${tr('cl_ago_min', {'n': '${diff.inMinutes}'})}';
            ...
            Text(
              device.isMoving
                  ...
                  : device.isInactive
                      ...
                      : '${device.statusAr}${_timeAgo(device.lastUpdate)}',
              style:
                  TextStyle(fontSize: 9, color: dotColor, fontFamily: 'Cairo'),
            ),
          ]),
        ]),
...
class _SubUserRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final badgeColor = user.isDealer
        ? const Color(0xFFC41E3A)
        : user.isSubDealer
            ? const Color(0xFF7C3AED)
            : const Color(0xFF1565C0);
    final badgeLabel = user.isDealer
        ? tr('tab_dealer')
        : user.isSubDealer
            ? tr('tab_subdealer')
            : tr('tab_client');
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Container(
            ...
            child: Text(badgeLabel,
                style: TextStyle(
                    fontSize: 8, color: badgeColor, fontFamily: 'Cairo')),
          ),
        ]),
      ),
    );
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          _UserAvatar(
              ...
              initials: u.firstLetter,
              radius: level == 0 ? 18 : 14,
              bgColor: colors[u.id % colors.length]),
          const SizedBox(width: 8),
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
          ...
          children: [
            ...
            Expanded(
                child: Column(
                  ...
                  Text(
                      user.isSubDealer
                          ? tr('cl_n_client_sub', {'n': '${user.clientCount}'})
                          : tr('cl_n_devices', {'n': '${user.deviceCount}'}),
                      style: const TextStyle(
                          fontSize: 9,
                          color: Color(0xFF8892A4),
                          fontFamily: 'Cairo')),
...
Color notifColorForType(String type, String? subtype) {
  if (type == 'ignitionOn' || type == 'geofenceEnter' || type == 'deviceOnline')
    return const Color(0xFF6BA539);
  if (type == 'ignitionOff' ||
      type == 'geofenceExit' ||
      type == 'deviceStopped' ||
      type == 'alert_parking') return const Color(0xFF6B7280);
  ...
  if (type == 'alarm') {
    if (subtype == 'lowBattery' || subtype == 'door')
...
String notifTypeLabel(String type, String? subtype) {
  switch (type) {
    case 'message':
      return tr('an_message');
    case 'ignitionOn':
      return tr('an_ign_on');
    case 'ignitionOff':
      return tr('an_ign_off');
    case 'deviceOverspeed':
      return tr('an_overspeed');
    case 'deviceOffline':
      return tr('an_offline');
    case 'deviceOnline':
      return tr('an_online');
    case 'deviceStopped':
      return tr('an_stopped');
    case 'alert_parking':
      return tr('an_parking');
    case 'alert_idle':
      return tr('an_idle');
    case 'deviceMoving':
      return tr('an_moving');
    case 'geofenceEnter':
      return tr('an_gf_enter');
    case 'geofenceExit':
      return tr('an_gf_exit');
    case 'alarm':
      switch (subtype) {
        case 'powerCut':
          return tr('an_power_cut');
        case 'lowBattery':
          return tr('an_low_battery');
        case 'door':
          return tr('an_door');
        case 'braking':
          return tr('an_braking');
        case 'acceleration':
          return tr('an_accel');
        case 'tow':
          return tr('an_tow');
...
String notifBodyText(Map<String, dynamic> n, String type, String? subtype) {
  ...
  if (type == 'message')
    return body; // رسالة إدارية: نص المستخدم كما هو (لا يُترجَم)
  ...
  final spdNum = n['speed'] is num
      ? (n['speed'] as num)
      : num.tryParse(n['speed']?.toString() ?? '');
  final spd = spdNum?.round();
  final timeStr =
      _fmtNotifTime((n['event_time'] ?? n['created_at'])?.toString());
  if (type == 'deviceOverspeed') {
    final m = RegExp(r'\(الحد[:\s]+(\d+)\)').firstMatch(body);
    final limit = m != null ? int.tryParse(m.group(1) ?? '') : null;
    if (spd != null && limit != null)
      return 'Overspeed $spd km/h (limit: $limit)';
    if (spd != null) return 'Overspeed $spd km/h';
    return timeStr;
  }
  if (type == 'alarm' &&
      (subtype == 'braking' ||
          subtype == 'harshBraking' ||
          subtype == 'hardBraking')) {
    return spd != null ? 'Harsh braking $spd km/h' : timeStr;
  }
  if (type == 'alarm' &&
      (subtype == 'acceleration' ||
          subtype == 'harshAcceleration' ||
          subtype == 'hardAcceleration')) {
...
void openAlertDetailFromNotif(BuildContext context, Map<String, dynamic> n) {
  ...
  final tid = n['traccar_id'] is int
      ? n['traccar_id'] as int
      : int.tryParse(n['traccar_id']?.toString() ?? '');
  final did = n['device_id'] is int
      ? n['device_id'] as int
      : int.tryParse(n['device_id']?.toString() ?? '');

  // البحث عن الجهاز من الـ provider (للحصول على الاسم/speedLimit/الموقع الاحتياطي)
  final provider = context.read<AppProvider>();
  DeviceModel? dev;
  for (final d in [...provider.devices, ...provider.inventory]) {
    if ((tid != null && d.traccarId == tid) || (did != null && d.id == did)) {
      dev = d;
      break;
    }
  }
  ...
  final lat = n['lat'] is num
      ? (n['lat'] as num).toDouble()
      : double.tryParse(n['lat']?.toString() ?? '');
  final lng = n['lng'] is num
      ? (n['lng'] as num).toDouble()
      : double.tryParse(n['lng']?.toString() ?? '');
  final spd = n['speed'] is num
      ? (n['speed'] as num).toDouble()
      : double.tryParse(n['speed']?.toString() ?? '');
  ...
  if (type == 'deviceOverspeed') {
    ...
    if (limitMatch != null) {
      resolvedSpeedLimit =
          int.tryParse(limitMatch.group(1) ?? '') ?? resolvedSpeedLimit;
    }
  }
...
class _MessagesScreenState extends State<MessagesScreen> {
  ...
  Future<void> _loadNotifications() async {
    ...
    try {
      ...
      if (mounted && r['success'] == true) {
        setState(() {
          _notifs =
              (r['notifications'] as List? ?? []).cast<Map<String, dynamic>>();
  ...
  Future<void> _markAllRead() async {
    ...
    try {
      final r =
          await ApiService.request('mark_notification_read', {'all': true});
      if (mounted && r['success'] == true) {
        setState(() {
      ...
      showDialog(
          ...
          builder: (dctx) => Directionality(
                textDirection:
                    I18n.isAr ? TextDirection.rtl : TextDirection.ltr,
                child: AlertDialog(
                  title: Text((n['title'] ?? tr('an_message')).toString(),
                      style: const TextStyle(
                          fontFamily: 'Cairo',
                          fontSize: 15,
  ...
  IconData _iconForType(String type, String? subtype) =>
      notifIconForType(type, subtype);

  Color _colorForType(String type, String? subtype) =>
  ...
  String _timeAgo(String? iso) {
    ...
    if (diff.inSeconds < 60)
      return tr(
          'ago_seconds', {'n': '${diff.inSeconds < 1 ? 1 : diff.inSeconds}'});
    if (diff.inMinutes < 60)
      return tr('ago_minutes', {'n': '${diff.inMinutes}'});
                    ...
                    child: Column(
                      ...
                      Text(notifTypeLabel(type, subtype),
                          style: TextStyle(
                              ...
                              fontFamily: 'Cairo')),
                      const SizedBox(height: 2),
                      Text(notifBodyText(n, type, subtype),
                          style: const TextStyle(
                              fontSize: 11,
...
class _AccountScreenState extends State<AccountScreen> {
      ...
      child: Column(
        children: [
                        ...
                        ElevatedButton(
                          ...
                          child: _saving
                              ? const SizedBox(
                                  ...
                                  height: 14,
                                  child: CircularProgressIndicator(
                                      color: Color(0xFFC41E3A), strokeWidth: 2))
                              : Text(tr('save'),
                                  style: const TextStyle(
                                      fontFamily: 'Cairo',
                                      fontSize: 12,
              ...
              Row(children: [
                ...
                Expanded(
                    child: Column(
                      ...
                      const SizedBox(height: 2),
                      Text(
                          '${user?.accountTypeAr ?? ''} - ${user?.username ?? ''}',
                          style: const TextStyle(
          ...
          Padding(
              ...
              child: Align(
                  alignment:
                      I18n.isAr ? Alignment.centerRight : Alignment.centerLeft,
                  child: Text(tr('acc_general'),
                      style: const TextStyle(
                          color: Color(0xFF8892A4),
                          fontSize: 10,
                          fontWeight: FontWeight.w500,
          ...
          _ActionCard(items: [
            _ActionItem(
                ...
                label: tr('acc_messages'),
                color: const Color(0xFF2563EB),
                bg: const Color(0x1A2563EB),
                onTap: () => _openAccountMessages(context)),
            if (context.read<AppProvider>().currentUser?.accountType ==
                    'admin' ||
                context.read<AppProvider>().currentUser?.accountType ==
                    'dealer')
            ...
            if (context.read<AppProvider>().currentUser?.accountType !=
              ...
              _ActionItem(
                  ...
                  onTap: () => _openCardNotifications(context)),
            _ActionItem(
                icon: Icons.notifications_active_outlined,
                label: tr('np_title'),
                color: const Color(0xFF6BA539),
                bg: const Color(0x1A6BA539),
                onTap: () => _openNotifPref(context)),
            _ActionItem(
                icon: Icons.schedule_outlined,
                label: tr('ae_title'),
                color: const Color(0xFFEF6C00),
                bg: const Color(0x1AEF6C00),
                onTap: () {
            ...
            if (context.read<AppProvider>().currentUser?.accountType !=
              ...
              _ActionItem(
                  ...
                  label: tr('acc_change_imei'),
                  color: const Color(0xFFF59E0B),
                  bg: const Color(0x1AF59E0B),
                  onTap: () => _changeImei(context)),
            _ActionItem(
                icon: Icons.language_outlined,
                label: tr('acc_app_lang'),
                color: const Color(0xFF0891B2),
                bg: const Color(0x1A0891B2),
            ...
            if (context.read<AppProvider>().currentUser?.accountType != 'admin')
              _ActionItem(
                  ...
                  label: tr('acc_provider'),
                  color: const Color(0xFF7C3AED),
                  bg: const Color(0x1A7C3AED),
                  onTap: () => _showServiceProvider(context)),
          ]),
          const SizedBox(height: 8),
          _ActionCard(items: [
            _ActionItem(
  ...
  void _showServiceProvider(BuildContext context) {
    Navigator.push(context,
        MaterialPageRoute(builder: (_) => const ServiceProviderScreen()));
  }
  ...
  void _openCardNotifications(BuildContext context) {
    ...
    Future<void> loadNotifs(StateSetter setS) async {
      ...
      try {
        final r = await ApiService.request(
            'get_messages', {'box': 'inbox', 'kind': 'notification'});
          ...
          builder: (_, sc) => Container(
            ...
            child: Column(children: [
              ...
              Padding(
                  ...
                  child: Row(children: [
                    tabBtn('in', tr('cn_received')),
                    tabBtn('out', tr('cn_sent'))
                  ])),
              const Divider(height: 1),
              Expanded(
                  child: loading
                                ...
                                return ListTile(
                                  ...
                                  subtitle: Text((n['body'] ?? '').toString(),
                                      style: const TextStyle(
                                          ...
                                          fontSize: 11,
                                          color: Color(0xFF6B7280))),
                                  trailing: Text(_fmtMsgTime(n['created_at']),
                                      style: const TextStyle(
                                          fontFamily: 'Cairo',
                                          fontSize: 9,
                                          color: Color(0xFF8892A4))),
  ...
  void _openBroadcast(BuildContext context) {
    ...
    Future<void> loadTargets(StateSetter setS, [int attempt = 0]) async {
      ...
      try {
        ...
        final arr = ((r['data'] ?? r['users']) as List? ?? [])
        ...
        if (attempt < 2) {
          await Future.delayed(const Duration(milliseconds: 600));
          return loadTargets(setS, attempt + 1);
        }
        setS(() => loading = false);
      }
    }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) {
        if (!started) {
          started = true;
          WidgetsBinding.instance
              .addPostFrameCallback((_) => loadTargets(setS));
        }
              ...
              child: Column(children: [
                ...
                Expanded(
                    child: loading
                        ...
                        : targets.isEmpty
                            ? Center(
                                child: Text(tr('bc_no_recipients'),
                                    style: const TextStyle(
                                        fontFamily: 'Cairo',
                                        color: Color(0xFF8892A4))))
                            : ListView.builder(
                ...
                Padding(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      ElevatedButton(
                        onPressed: (sending || targets.isEmpty)
                            ? null
                            : () async {
                                if (msgCtrl.text.trim().isEmpty) return;
                                setS(() => sending = true);
                                final all =
                                    sel.isEmpty || sel.length == targets.length;
                                bool sentOk = false;
                                String? sendErr;
                                try {
                                  final r = await ApiService.request(
                                      ...
                                      all
                                          ...
                                          : {
                                              'body': msgCtrl.text.trim(),
                                              'target': 'some',
                                              'recipients': sel.toList()
                                            });
                                  sentOk = r['success'] == true;
                                  sendErr = r['error'] as String?;
                                } catch (_) {}
                                if (ctx.mounted) {
                                  Navigator.pop(ctx);
                                  // كانت تعلن «تم الإرسال» حتى مع انقطاع النت فيظن المرسِل أن رسالته وصلت
                                  ScaffoldMessenger.of(context)
                                      .showSnackBar(SnackBar(
                                    content: Text(
                                        sentOk
          ...
          child: Column(
              ...
              children: [
                ...
                loading
                    ...
                    : SwitchListTile(
                        value: enabled,
                        activeThumbColor: const Color(0xFF6BA539),
                        contentPadding: EdgeInsets.zero,
                        title: Text(
                            I18n.isAr
                                ? (enabled ? 'مفعّل' : 'معطّل')
  ...
  void _openMessagesHub(BuildContext context) {
    ...
    Future<void> loadConvs(StateSetter setS, [int attempt = 0]) async {
      ...
      try {
        ...
        for (final m in all) {
          ...
          if (existing == null ||
              created.compareTo(existing['last_time'].toString()) > 0) {
            byParty[otherId] = {
              'id': otherId,
              'name': otherName,
        ...
        final list = byParty.values.toList()
          ..sort((a, b) =>
              b['last_time'].toString().compareTo(a['last_time'].toString()));
        setS(() {
          convs = list;
          loading = false;
        });
        ...
        if (attempt < 2) {
          await Future.delayed(const Duration(milliseconds: 600));
          return loadConvs(setS, attempt + 1);
        }
        setS(() => loading = false);
      }
    }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) {
        if (!started) {
          started = true;
          WidgetsBinding.instance.addPostFrameCallback((_) => loadConvs(setS));
        }
        return DraggableScrollableSheet(
          initialChildSize: 0.85,
          maxChildSize: 0.95,
                              ...
                              itemBuilder: (_, i) {
                                ...
                                return ListTile(
                                  ...
                                  onTap: () => _openConversation(context,
                                      c['id'].toString(), c['name'] as String),
                                );
                              },
                            )),
            ]),
  ...
  void _openConversation(
      BuildContext context, String partyId, String partyName) {
    final sendCtrl = TextEditingController();
    final myId = context.read<AppProvider>().currentUser?.id.toString();
    List<Map<String, dynamic>> msgs = [];
    bool loading = true, sending = false;
    ...
    Future<void> loadConv(StateSetter setS, [int attempt = 0]) async {
      ...
      try {
        ...
        dedup.sort((a, b) => (a['created_at'] ?? '')
            .toString()
            .compareTo((b['created_at'] ?? '').toString()));
        setS(() {
          msgs = dedup.reversed.toList();
          loading = false;
        });
        ...
        if (attempt < 2) {
          await Future.delayed(const Duration(milliseconds: 600));
          return loadConv(setS, attempt + 1);
        }
        setS(() => loading = false);
      }
    }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) {
        if (!started) {
          started = true;
          WidgetsBinding.instance.addPostFrameCallback((_) => loadConv(setS));
        }
        return DraggableScrollableSheet(
          initialChildSize: 0.85,
          maxChildSize: 0.95,
              ...
              Padding(
                  ...
                  child: Row(children: [
                    CircleAvatar(
                        radius: 17,
                        backgroundColor: const Color(0xFF2563EB),
                        child: Text(partyName.isNotEmpty ? partyName[0] : '?',
                            style: const TextStyle(
                                color: Colors.white,
                                fontFamily: 'Cairo',
                                fontWeight: FontWeight.bold))),
  ...
  void _openProviderMessages(BuildContext context) {
    ...
    Future<void> loadAll(StateSetter setS, [int attempt = 0]) async {
      ...
      try {
        ...
        if (pid != null)
          ApiService.request('mark_messages_read', {'peer_id': pid});
      } catch (_) {
        if (attempt < 2) {
          await Future.delayed(const Duration(milliseconds: 600));
          return loadAll(setS, attempt + 1);
        }
        setS(() => loading = false);
      }
    }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) {
        if (!started) {
          started = true;
          WidgetsBinding.instance.addPostFrameCallback((_) => loadAll(setS));
        }
        final providerName = provider?['full_name'] ??
            provider?['username'] ??
            tr('pm_provider');
        return DraggableScrollableSheet(
          initialChildSize: 0.85,
                  ...
                  GestureDetector(
                    onTap: sending
                        ...
                        : () async {
                            ...
                            if (provider?['id'] != null)
                              payload['receiver_id'] = provider!['id'];
  ...
  Future<void> _save() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(ok ? tr('cl_data_saved') : tr('cl_save_failed'),
            style: const TextStyle(fontFamily: 'Cairo')),
        backgroundColor: ok ? const Color(0xFF6BA539) : const Color(0xFFC41E3A),
      ));
    }
  ...
  Future<void> _logout() async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(tr('cl_logout_title'),
            style: const TextStyle(fontFamily: 'Cairo')),
        content: Text(tr('cl_logout_q'),
            style: const TextStyle(fontFamily: 'Cairo')),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(ctx, false),
          ...
          TextButton(
              onPressed: () => Navigator.pop(ctx, true),
              child: Text(tr('cl_logout_btn'),
                  style: const TextStyle(
                      color: Color(0xFFC41E3A), fontFamily: 'Cairo'))),
        ],
      ),
    );
    if (confirm == true && mounted) {
...
class _ServiceProviderScreenState extends State<ServiceProviderScreen> {
  ...
  void _copy(String value) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content:
          Text(tr('sp_copied'), style: const TextStyle(fontFamily: 'Cairo')),
      backgroundColor: const Color(0xFF6BA539),
      duration: const Duration(seconds: 1),
    ));
  }
          ...
          builder: (c, snap) {
            ...
            final avatar = rawAvatar.length > 1
                ? (rawAvatar.startsWith('http')
                    ? rawAvatar
                    : 'https://himaya-track.com$rawAvatar')
                : '';
              ...
              child: Column(children: [
                ...
                Padding(
                  ...
                  child: Column(children: [
                    ...
                    _ProviderRow(
                        icon: Icons.chat,
                        iconWidget: SvgPicture.string(_kWhatsappSvg,
                            width: 20, height: 20),
                        label: tr('sp_whatsapp'),
                        value: phone.isNotEmpty ? phone : tr('sp_not_set'),
                        onTap: phone.isNotEmpty
                            ? () => _launch(
                                'https://wa.me/${phone.replaceAll(RegExp(r'[^0-9]'), '').replaceFirst(RegExp(r'^0+'), '')}')
                            : null,
                        onCopy: phone.isNotEmpty ? () => _copy(phone) : null),
                    _ProviderRow(
                        icon: Icons.email_outlined,
                        label: tr('sp_email'),
                        value: email.isNotEmpty ? email : tr('sp_not_set'),
                    ...
                    _ProviderRow(
                        icon: Icons.language_outlined,
                        label: tr('sp_website'),
                        value: website.isNotEmpty ? website : tr('sp_not_set'),
                        onTap: website.isNotEmpty
                            ...
                            : null,
                        onCopy:
                            website.isNotEmpty ? () => _copy(website) : null),
                  ]),
                ),
              ]),
            );
            ...
            Container(
              ...
              child: Image.asset('assets/images/logo.png',
                  ...
                  errorBuilder: (_, __, ___) => const Icon(Icons.location_on,
                      color: Color(0xFFC41E3A), size: 80)),
            ),
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Map<String, String> _recentPw = {}; // اسم المستخدم ← كلمة المرور المحفوظة

  static const String _kRecentUsersKey = 'recent_usernames';
  static const String _kRecentPwKey = 'recent_pw';
  ...
  Future<void> _loadRecent() async {
    ...
    try {
      final raw = prefs.getString(_kRecentPwKey);
      if (raw != null && raw.isNotEmpty) {
        pw = Map<String, String>.from(jsonDecode(raw) as Map);
      }
    } catch (_) {}
    if (mounted)
      setState(() {
        _recentUsernames = list;
        _recentPw = pw;
      });
  }

  Future<void> _saveRecent(String username, [String? password]) async {
    ...
    // كلمة المرور تُحفظ ليفتح الحساب بضغطة. تُحذف عند خروجه من آخر ٣ حسابات.
    final pw = Map<String, String>.from(_recentPw);
    if (password != null && password.isNotEmpty) pw[username] = password;
    pw.removeWhere((k, v) => !list.contains(k));
    await prefs.setString(_kRecentPwKey, jsonEncode(pw));
  ...
  Future<void> _removeRecent(String username) async {
    ...
    list.removeWhere((u) => u == username);
    await prefs.setStringList(_kRecentUsersKey, list);
    final pw = Map<String, String>.from(_recentPw)..remove(username);
    await prefs.setString(_kRecentPwKey, jsonEncode(pw));
    if (mounted)
      setState(() {
        _recentUsernames = list;
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: Theme.of(context).colorScheme.surface,
                        borderRadius: BorderRadius.circular(16)),
                    child: provider.needsPasswordChange
                        ? _buildPwChangeColumn(provider)
                        : provider.needsTwoFactor
                            ? _buildOtpColumn(provider)
                            : Column(
                                children: [
                                  TextField(
                                              ...
                                              shape: RoundedRectangleBorder(
                                                  borderRadius:
                                                      BorderRadius.circular(
                                                          10)),
                                              onSelected: (u) {
                                                final saved = _recentPw[u];
                                                setState(() {
                                                  _userCtrl.text = u;
                                                  _passCtrl.text = saved ?? '';
                                                });
                                                // كلمة مرور محفوظة => دخول مباشر بلا كتابة
                                                if (saved != null &&
                                                    saved.isNotEmpty) _login();
                                              },
                                              itemBuilder: (_) =>
                                                  _recentUsernames
                                                      .map(
  ...
  Future<void> _login() async {
    ...
    final pwUsed = _passCtrl.text;
    final ok = await context.read<AppProvider>().login(username, pwUsed);
    if (ok && mounted) {
      await _saveRecent(username, pwUsed);
  ...
  Widget _buildOtpColumn(AppProvider provider) {
    return Column(
      ...
      children: [
        ...
        Text(
            I18n.isAr
                ? 'تم إرسال رمز تحقق المالك عبر تيليجرام'
                : 'An owner verification code was sent via Telegram',
            textAlign: TextAlign.center,
            style: const TextStyle(
                fontFamily: 'Cairo',
                fontSize: 13,
        ...
        if (provider.error != null) ...[
          ...
          Text(provider.error!,
              textAlign: TextAlign.center,
              style: const TextStyle(
                  color: Color(0xFFC41E3A), fontSize: 12, fontFamily: 'Cairo')),
        ],
        const SizedBox(height: 16),
        ElevatedButton(
  ...
  Future<void> _verify() async {
    ...
    final ok =
        await context.read<AppProvider>().verifyTwoFactor(_otpCtrl.text.trim());
    if (ok && mounted) {
      await _saveRecent(_userCtrl.text.trim(), _passCtrl.text);
      _otpCtrl.clear();
      FirebaseMessaging.instance.getToken().then((fcmToken) {
        if (fcmToken != null) ApiService.saveFcmToken(fcmToken);
      });
  ...
  Widget _buildPwChangeColumn(AppProvider provider) {
    InputDecoration dec(String h) => InputDecoration(
        hintText: h, prefixIcon: const Icon(Icons.lock_outline));
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        ...
        Text(
            I18n.isAr
                ? 'لأول دخول، اختر كلمة مرور جديدة خاصة بك'
                : 'First login — please set your own new password',
            textAlign: TextAlign.center,
            style: const TextStyle(
                fontFamily: 'Cairo', fontSize: 11, color: Colors.grey)),
        const SizedBox(height: 12),
        ...
        TextField(
            ...
            autofocus: true,
            decoration:
                dec(I18n.isAr ? 'كلمة المرور الجديدة' : 'New password')),
        const SizedBox(height: 10),
        TextField(
            ...
            decoration:
                dec(I18n.isAr ? 'تأكيد كلمة المرور' : 'Confirm password')),
        if (provider.error != null) ...[
          const SizedBox(height: 8),
          Text(provider.error!,
              textAlign: TextAlign.center,
        ...
        ElevatedButton(
          ...
          child: provider.isLoading
              ? const SizedBox(
                  ...
                  child: CircularProgressIndicator(
                      color: Colors.white, strokeWidth: 2))
              : Text(I18n.isAr ? 'حفظ ودخول' : 'Save & continue',
                  style: const TextStyle(
                      fontSize: 14,
                      fontFamily: 'Cairo',
                      fontWeight: FontWeight.w600)),
  ...
  Future<void> _submitNewPassword() async {
    ...
    if (np != cp) {
      _snack(
          I18n.isAr ? 'كلمتا المرور غير متطابقتين' : 'Passwords do not match');
      return;
    }
    final ok = await context.read<AppProvider>().submitNewPassword(np);
    if (ok && mounted) {
      await _saveRecent(_userCtrl.text.trim(), np); // الجديدة لا القديمة
      _newPwCtrl.clear();
      _confirmPwCtrl.clear();
      FirebaseMessaging.instance.getToken().then((fcmToken) {
        if (fcmToken != null) ApiService.saveFcmToken(fcmToken);
              ...
              child: Container(
                ...
                child: Column(children: [
                  Text(t.label,
                      style: TextStyle(
                          fontSize: 10,
                          color: isOn ? Colors.white : const Color(0xFF8892A4),
                          fontFamily: 'Cairo')),
                  Text('${counts[t.id] ?? 0}',
                      style: TextStyle(
                          fontSize: 8,
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(label,
            style: const TextStyle(
                fontSize: 9, color: Color(0xDDFFFFFF), fontFamily: 'Cairo')),
      ]),
    ));
  }
}
  ...
  Widget build(BuildContext context) {
    return Expanded(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(label,
            style: const TextStyle(
                fontSize: 7.5, color: Color(0xCCFFFFFF), fontFamily: 'Cairo'),
            textAlign: TextAlign.center,
            maxLines: 2),
      ]),
    ));
...
extension _AccountScreenMethods on _AccountScreenState {
      ...
      builder: (ctx) => SafeArea(
        ...
        if (user?.avatarUrl != null)
          ListTile(
              ...
              onTap: () {
                Navigator.pop(ctx);
                _viewAvatar(context, user!.avatarUrl!);
              }),
        ListTile(
            leading:
                const Icon(Icons.camera_alt_outlined, color: Color(0xFFC41E3A)),
        ...
        if (user?.avatarUrl != null)
          ListTile(
              ...
              title: Text(tr('cl_delete_photo'),
                  style: const TextStyle(
                      fontFamily: 'Cairo', color: Color(0xFFEF5350))),
              onTap: () {
                Navigator.pop(ctx);
  ...
  Future<void> _deleteAvatar(BuildContext context) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
              r['success'] == true ? tr('cl_photo_deleted') : tr('cl_fail'),
              style: const TextStyle(fontFamily: 'Cairo')),
          backgroundColor: r['success'] == true
              ? const Color(0xFF6BA539)
              : const Color(0xFFC41E3A)));
      ...
      if (mounted) {
        ...
        if (r['success'] == true) {
          ...
          if (newUrl != null && mounted)
            setState(() => _localAvatarUrl = newUrl);
                  ...
                  actions: [
                    ...
                    ElevatedButton(
                        ...
                        onPressed: sv
                            ? null
                            : () async {
                                if (o.text.isEmpty || n.text.isEmpty) return;
                                if (n.text != cf.text) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                          content: Text(tr('pw_mismatch'),
                                              style: const TextStyle(
                                                  fontFamily: 'Cairo')),
                                          backgroundColor:
                                              const Color(0xFFC41E3A)));
                  ...
                  actions: [
                    ...
                    ElevatedButton(
                        ...
                        onPressed: sv
                            ? null
                            : () async {
                                if (oc.text.trim().isEmpty ||
                                    nc.text.trim().isEmpty) {
                                  ss(() => err = tr('im_enter_both'));
                                  return;
                                }
                                ss(() => sv = true);
                                final r = await ApiService.request(
                                    'change_imei', {
                                  'old_imei': oc.text.trim(),
                                  'new_imei': nc.text.trim(),
                                  'model': model
                                });
                                ss(() => sv = false);
                                if (r['success'] == true) {
                                  ...
                                  if (mounted)
                                    ScaffoldMessenger.of(context).showSnackBar(
                                        SnackBar(
                                            content: Text(tr('im_changed'),
                                                style: const TextStyle(
                                                    fontFamily: 'Cairo')),
                                            backgroundColor:
                                                const Color(0xFF6BA539)));
                                } else {
                                  ss(() => err = r['error'] ?? tr('im_failed'));
                                }
                              },
                        child: sv
                            ? const SizedBox(
  ...
  void _changeLanguage(BuildContext context) {
    ...
    showDialog(
        ...
        builder: (ctx) => AlertDialog(
              ...
              content: Column(mainAxisSize: MainAxisSize.min, children: [
                ListTile(
                    leading: const Text('عر', style: TextStyle(fontSize: 20)),
                    title: Text(tr('arabic'),
                        style: const TextStyle(fontFamily: 'Cairo')),
                    trailing: cur == 'ar'
                        ? const Icon(Icons.check, color: Color(0xFFC41E3A))
                        : null,
                    onTap: () {
                      if (cur == 'ar') {
                        Navigator.pop(ctx);
                      ...
                      _confirmLang(context, ctx, prov, 'ar');
                    }),
                ListTile(
                    ...
                    onTap: () {
                      ...
                      _confirmLang(context, ctx, prov, 'en');
                    }),
              ]),
              actions: [
                TextButton(
              ...
              content: Column(mainAxisSize: MainAxisSize.min, children: [
                ListTile(
                  ...
                  title: Text(tr('theme_system'),
                      style: const TextStyle(fontFamily: 'Cairo')),
                  trailing: cur == ThemeMode.system
                      ? const Icon(Icons.check, color: Color(0xFFC41E3A))
                      : null,
                  onTap: () {
                    prov.setThemeMode(ThemeMode.system);
                    Navigator.pop(ctx);
                  },
                ),
                ListTile(
                  ...
                  title: Text(tr('theme_light'),
                      style: const TextStyle(fontFamily: 'Cairo')),
                  trailing: cur == ThemeMode.light
                      ? const Icon(Icons.check, color: Color(0xFFC41E3A))
                      : null,
                  onTap: () {
                    prov.setThemeMode(ThemeMode.light);
                    Navigator.pop(ctx);
                  },
                ),
                ListTile(
                  ...
                  title: Text(tr('theme_dark'),
                      style: const TextStyle(fontFamily: 'Cairo')),
                  trailing: cur == ThemeMode.dark
                      ? const Icon(Icons.check, color: Color(0xFFC41E3A))
                      : null,
                  onTap: () {
                    prov.setThemeMode(ThemeMode.dark);
                    Navigator.pop(ctx);
                  },
                ),
              ]),
  ...
  void _confirmLang(BuildContext context, BuildContext pickerCtx,
      AppProvider prov, String code) {
    showDialog(
        context: context,
        builder: (c2) => AlertDialog(
              title: Text(tr('lang_confirm_title'),
                  style: const TextStyle(fontFamily: 'Cairo', fontSize: 14)),
              content: Text(tr('lang_confirm_msg'),
        ...
        builder: (ctx) => AlertDialog(
              ...
              content: Container(
                  ...
                  decoration: BoxDecoration(
                      color: Theme.of(context).scaffoldBackgroundColor,
                      borderRadius: BorderRadius.circular(8)),
                  child: const Text('H.Track - https://himaya-track.com',
              ...
              actions: [
                ...
                ElevatedButton.icon(
                    ...
                    label: Text(tr('sh_copy'),
                        style: const TextStyle(fontFamily: 'Cairo')),
                    onPressed: () {
                      Navigator.pop(ctx);
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                          content: Text(tr('sh_copied'),
                              style: const TextStyle(fontFamily: 'Cairo')),
                          backgroundColor: const Color(0xFF6BA539)));
                    })
              ],
...
class _EditDeviceSheetState extends State<_EditDeviceSheet>
  ...
  static List<Map<String, String>> get _vIcons => [
        {'key': 'car', 'label': tr('vt_car')},
        {'key': 'boat', 'label': tr('veh_boat')},
        {'key': 'pickup', 'label': tr('vt_pickup')},
        {'key': 'van', 'label': tr('vt_van')},
        {'key': 'bus', 'label': tr('vt_bus')},
        {'key': 'truck', 'label': tr('vt_truck')},
        {'key': 'motorcycle', 'label': tr('vt_motorcycle')},
        {'key': 'tuk_tuk', 'label': tr('vt_tuktuk')},
        {'key': 'excavator', 'label': tr('veh_excavator')}
      ];
  static const _vIconData = {
  ...
  Future<void> _load() async {
    try {
      final d =
          <String, dynamic>{}; // نستخدم widget.device مباشرة بدون API call
      if (mounted) {
        setState(() {
          _model = widget.device.deviceType;
          _icon = d['icon'] ?? widget.device.icon;
          _name = TextEditingController(text: d['name'] ?? widget.device.name);
          _plate = TextEditingController(text: d['plate_number'] ?? '');
          _iccid = TextEditingController(text: widget.device.iccid);
          _phone = TextEditingController(text: widget.device.simPhone);
          _installPerson =
              TextEditingController(text: d['install_person'] ?? '');
          _installTime = TextEditingController(text: d['install_time'] ?? '');
          _installCompany =
              TextEditingController(text: d['install_company'] ?? '');
          _installLocation =
              TextEditingController(text: d['install_location'] ?? '');
          _installAddress =
              TextEditingController(text: d['install_address'] ?? '');
          _notes = TextEditingController(text: d['notes'] ?? '');
          _driverName = TextEditingController(text: d['driver_name'] ?? '');
          _driverMobile = TextEditingController(text: d['driver_mobile'] ?? '');
          _engineNumber = TextEditingController(text: d['engine_number'] ?? '');
          _vehicleBrand = TextEditingController(text: d['vehicle_brand'] ?? '');
          _vehicleColor = TextEditingController(text: d['vehicle_color'] ?? '');
          _driverId = TextEditingController(text: d['driver_id'] ?? '');
          _permitNo = TextEditingController(text: d['permit_no'] ?? '');
          _permitCoverage =
              TextEditingController(text: d['permit_coverage'] ?? '');
          _speedLimit = TextEditingController(
              text: (widget.device.speedLimit).toString());
          _aEOn = widget.device.alertEngineOn == 1;
          _aEOff = widget.device.alertEngineOff == 1;
          _aOvr = widget.device.alertOverspeed == 1;
          _aIdl = widget.device.alertIdle == 1;
          _aNs = widget.device.alertNoSignal == 1;
          _aPk = widget.device.alertParking == 1;
          _aDoor = widget.device.alertDoor == 1;
          _aPow = widget.device.alertPowerCut == 1;
          _aLowBat = widget.device.alertLowBattery == 1;
          _aSos = widget.device.alertSos == 1;
          _aGeo = widget.device.alertGeofence == 1;
          _aTow = widget.device.alertTowing == 1;
          _aHBrk = widget.device.alertHarshBrake == 1;
          _aHAcc = widget.device.alertHarshAccel == 1;
          _expiryDate = widget.device.subscriptionEnd;
          _expiryCtrl.text = _fmtDate(_expiryDate);
          _loading = false;
        });
      }
    } catch (_) {
      if (mounted)
        setState(() {
          ...
          _plate = _installPerson = _installTime = _installCompany =
              _installLocation = _installAddress = _notes = _driverName =
                  _driverMobile = _engineNumber = _vehicleBrand =
                      _vehicleColor = _driverId =
                          _permitNo = _permitCoverage = TextEditingController();
          _iccid = TextEditingController(text: widget.device.iccid);
          _phone = TextEditingController(text: widget.device.simPhone);
          _expiryDate = widget.device.subscriptionEnd;
          _expiryCtrl.text = _fmtDate(_expiryDate);
  ...
  Future<void> _save() async {
    ...
    if (_aOvr) {
      ...
      if (lim <= 0) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(tr('speed_limit_required'),
              style: const TextStyle(fontFamily: 'Cairo')),
          backgroundColor: const Color(0xFFC41E3A),
        ));
        return;
    ...
    setState(() => _saving = true);
    final provider = context.read<AppProvider>();
    final r = await ApiService.request('update_device', {
      'id': widget.device.traccarId,
      ...
      'install_person': _installPerson.text.trim(),
      'install_time': _installTime.text.trim(),
      'install_company': _installCompany.text.trim(),
      'install_location': _installLocation.text.trim(),
      'install_address': _installAddress.text.trim(),
      'notes': _notes.text.trim(),
      'driver_name': _driverName.text.trim(),
      'driver_mobile': _driverMobile.text.trim(),
      'engine_number': _engineNumber.text.trim(),
      'vehicle_brand': _vehicleBrand.text.trim(),
      'vehicle_color': _vehicleColor.text.trim(),
      'driver_id': _driverId.text.trim(),
      'permit_no': _permitNo.text.trim(),
      'permit_coverage': _permitCoverage.text.trim(),
      'expiry_date': _expiryDate != null ? _fmtDate(_expiryDate) : '',
      'speed_limit': int.tryParse(_speedLimit.text) ?? 100,
      'alert_engine_on': _aEOn ? 1 : 0,
      'alert_engine_off': _aEOff ? 1 : 0,
      'alert_overspeed': _aOvr ? 1 : 0,
      'alert_idle': _aIdl ? 1 : 0,
      'alert_no_signal': _aNs ? 1 : 0,
      'alert_parking': _aPk ? 1 : 0,
      'alert_door': _aDoor ? 1 : 0,
      'alert_power_cut': _aPow ? 1 : 0,
      'alert_low_battery': _aLowBat ? 1 : 0,
      'alert_sos': _aSos ? 1 : 0,
      'alert_geofence': _aGeo ? 1 : 0,
      'alert_towing': _aTow ? 1 : 0,
      'alert_harsh_brake': _aHBrk ? 1 : 0,
      'alert_harsh_accel': _aHAcc ? 1 : 0
    });
    setState(() => _saving = false);
    if (!mounted) return;
    ...
    if (r['success'] == true) {
      ...
      d.alertEngineOn = _aEOn ? 1 : 0;
      d.alertEngineOff = _aEOff ? 1 : 0;
      d.alertOverspeed = _aOvr ? 1 : 0;
      d.alertIdle = _aIdl ? 1 : 0;
      d.alertNoSignal = _aNs ? 1 : 0;
      d.alertParking = _aPk ? 1 : 0;
      d.alertDoor = _aDoor ? 1 : 0;
      d.alertPowerCut = _aPow ? 1 : 0;
      d.alertLowBattery = _aLowBat ? 1 : 0;
      d.alertSos = _aSos ? 1 : 0;
      d.alertGeofence = _aGeo ? 1 : 0;
      d.alertTowing = _aTow ? 1 : 0;
      d.alertHarshBrake = _aHBrk ? 1 : 0;
      d.alertHarshAccel = _aHAcc ? 1 : 0;
      d.speedLimit = int.tryParse(_speedLimit.text) ?? d.speedLimit;
      widget.onSaved();
      provider.loadDevices();
    }
  }
  ...
  // ── helpers theme-aware ──────────────────────────────────────────────────────
  Widget _inp(String l, TextEditingController c,
      {TextInputType? kb, int ml = 1}) {
  ...
  Widget _r2(Widget a, Widget b) => Row(children: [
        Expanded(child: a),
        const SizedBox(width: 8),
        Expanded(child: b)
      ]);
  ...
  bool get _canEditExpiry {
    ...
    return (widget.device.subscriptionType ?? '')
        .toLowerCase()
        .contains('lifetime');
  }
          ...
          child: Container(
            ...
            child: Column(children: [
              ...
              Padding(
                  ...
                  child: Row(children: [
                    const Icon(Icons.edit, size: 18, color: Color(0xFF1565C0)),
                    const SizedBox(width: 8),
                    Expanded(
                        child: Text(
                            '${tr('edit_device')} - ${widget.device.name}',
                            style: const TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.w600,
                                fontFamily: 'Cairo'))),
              ...
              Directionality(
                  ...
                  child: TabBar(
                      ...
                      tabs: [
                        Tab(text: tr('tab_basic')),
                        Tab(text: tr('tab_icon')),
                        Tab(text: tr('tab_install')),
                        Tab(text: tr('tab_alerts')),
                        Tab(text: tr('tab_geofence')),
                      ...
                      : TabBarView(controller: _tab, children: [
                          SingleChildScrollView(
                              ...
                              child: Builder(builder: (ctx) {
                                ...
                                Widget topField(String lbl, Widget child) =>
                                    Padding(
                                        padding:
                                  ...
                                  _r2(
                                      Padding(
                                          ...
                                          child: ModelPickerField(
                                              value: _models.contains(_model)
                                                  ? _model
                                                  : _models.first,
                                              label: tr('device_type'),
                                              includeOther: true,
                                              onChanged: (v) =>
                                                  setState(() => _model = v))),
                                      _inp(tr('plate_number'), _plate)),
                                  _r2(
                                      _inp('ICCID', _iccid,
                                          kb: TextInputType.number),
                                      _inp(tr('sim_number'), _phone,
                                          kb: TextInputType.phone)),
                                  _r2(
                                    _ro(tr('activation_date'),
                                        _fmtDate(widget.device.activatedAt)),
                                    topField(
                                        tr('expiry_date'),
                                        GestureDetector(
                                            onTap: _canEditExpiry
                                                ? () async {
                          ...
                          Builder(builder: (ctx) {
                            final divC = Theme.of(ctx).dividerColor;
                            final surf = Theme.of(ctx).scaffoldBackgroundColor;
                            return SingleChildScrollView(
                                padding: const EdgeInsets.all(16),
                                child: GridView.count(
                                    ...
                                    crossAxisSpacing: 10,
                                    mainAxisSpacing: 10,
                                    children: _vIcons.map((v) {
                                      final s = _icon == v['key'];
                                      return GestureDetector(
                                          onTap: () =>
                                              setState(() => _icon = v['key']!),
                                          child: Container(
                                              decoration: BoxDecoration(
                                                  color: s
                                                      ? const Color(0xFF1565C0)
                          ...
                          SingleChildScrollView(
                              padding: const EdgeInsets.all(16),
                              child: Column(children: [
                                _r2(_inp(tr('install_person'), _installPerson),
                                    _inp(tr('install_time'), _installTime)),
                                _r2(
                                    _inp(
                                        tr('install_company'), _installCompany),
                                    _inp(tr('install_location'),
                                        _installLocation)),
                                _inp(tr('install_address'), _installAddress),
                                _inp(tr('notes'), _notes, ml: 3)
                              ])),
                          Builder(builder: (ctx) {
                            final onS = Theme.of(ctx).colorScheme.onSurface;
                            return SingleChildScrollView(
                                ...
                                child: Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 8),
                                      Wrap(children: [
                                        _chk(tr('al_engine_on'), _aEOn,
                                            (v) => setState(() => _aEOn = v)),
                                        _chk(tr('al_engine_off'), _aEOff,
                                            (v) => setState(() => _aEOff = v)),
                                        _chk(tr('al_overspeed'), _aOvr, (v) {
                                          setState(() {
                                            _aOvr = v;
                                            if (v &&
                                                _speedLimit.text.trim().isEmpty)
                                              _speedLimit.text = '';
                                          });
                                        }),
                                        _chk(tr('al_idle'), _aIdl,
                                            (v) => setState(() => _aIdl = v)),
                                        _chk(tr('al_no_signal'), _aNs,
                                            (v) => setState(() => _aNs = v)),
                                        _chk(tr('al_parking'), _aPk,
                                            (v) => setState(() => _aPk = v)),
                                        _chk(tr('al_power_cut'), _aPow,
                                            (v) => setState(() => _aPow = v)),
                                        _chk(tr('al_sos'), _aSos,
                                            (v) => setState(() => _aSos = v)),
                                        _chk(tr('al_door'), _aDoor,
                                            (v) => setState(() => _aDoor = v)),
                                        _chk(
                                            tr('al_low_battery'),
                                            _aLowBat,
                                            (v) =>
                                                setState(() => _aLowBat = v)),
                                        _chk(tr('al_towing'), _aTow,
                                            (v) => setState(() => _aTow = v)),
                                        _chk(tr('al_geofence'), _aGeo,
                                            (v) => setState(() => _aGeo = v)),
                                        _chk(tr('al_harsh_brake'), _aHBrk,
                                            (v) => setState(() => _aHBrk = v)),
                                        _chk(tr('al_harsh_accel'), _aHAcc,
                                            (v) => setState(() => _aHAcc = v))
                                      ]),
                                      const SizedBox(height: 12),
                                      AnimatedSize(
                          ...
                          SingleChildScrollView(
                              ...
                              child: Column(children: [
                                _r2(
                                    _inp(tr('driver_name'), _driverName),
                                    _inp(tr('driver_mobile'), _driverMobile,
                                        kb: TextInputType.phone)),
                                _r2(_inp(tr('engine_number'), _engineNumber),
                                    _inp(tr('vehicle_brand'), _vehicleBrand)),
                                _r2(_inp(tr('vehicle_color'), _vehicleColor),
                                    _inp('Driver IC', _driverId)),
                                _r2(_inp('Permit No', _permitNo),
                                    _inp('Permit Coverage', _permitCoverage))
                              ])),
                        ])),
              Padding(
                  padding: EdgeInsets.fromLTRB(
          ...
          builder: (_) => Directionality(
                ...
                child: Scaffold(
                  appBar: AppBar(
                    ...
                    title: Text('${tr('tab_geofence')} - ${device.name}',
                        style: const TextStyle(
                            fontFamily: 'Cairo',
                            fontSize: 15,
                            fontWeight: FontWeight.w600)),
                  ...
                  body: _DeviceGeofenceTab(
                    ...
                    onEnableAlert: () {
                      ApiService.request('update_device',
                          {'id': device.traccarId, 'alert_geofence': 1});
                    },
                  ),
                ),
              )));
...
class _DeviceGeofenceTab extends StatefulWidget {
  ...
  const _DeviceGeofenceTab(
      {required this.device,
      required this.alertEnabled,
      required this.onEnableAlert});
  @override
  State<_DeviceGeofenceTab> createState() => _DeviceGeofenceTabState();
}
...
class _DeviceGeofenceTabState extends State<_DeviceGeofenceTab> {
  ...
  Widget _gchip(String l, Color fg, Color bg) => Container(
  ...
  Future<void> _loadLinked() async {
    ...
    final r = await ApiService.request(
        'device_geofences', {'device_id': widget.device.id});
  ...
  Future<void> _unlink(int gfId, String name) async {
    final ok = await showDialog<bool>(
        ...
        builder: (c) => AlertDialog(
                title: Text(tr('gf_remove_link'),
                    style: const TextStyle(
                        fontFamily: 'Cairo',
                        fontSize: 15,
                        fontWeight: FontWeight.w600)),
                content: Text(tr('gf_remove_q', {'n': name}),
                    style: const TextStyle(fontFamily: 'Cairo', fontSize: 13)),
                actions: [
                  TextButton(
                  ...
                  TextButton(
                      ...
                      child: Text(tr('remove'),
                          style: const TextStyle(
                              fontFamily: 'Cairo', color: Color(0xFFC41E3A))))
                ]));
    if (ok != true) return;
    setState(() => _busy = true);
    await ApiService.request('unlink_device_geofence',
        {'device_id': widget.device.id, 'geofence_id': gfId});
    await _loadLinked();
  ...
  Future<void> _openSettings(Map<String, dynamic> gs) async {
    final res = await showModalBottomSheet<Map<String, int>>(
        ...
        isScrollControlled: true,
        backgroundColor: Colors.transparent,
        builder: (_) => _GeofenceSettingsSheet(
              name: (gs['name']?.toString().trim().isNotEmpty == true)
                  ? gs['name'].toString()
                  : '${tr('tab_geofence')} ${_i(gs['geofence_id'])}',
              evEnter: _i(gs['ev_enter']) == 1,
              evExit: _i(gs['ev_exit']) == 1,
              evSpeed: _i(gs['ev_speed']) == 1,
              stopEngine: _i(gs['stop_engine']) == 1,
              notifApp: _i(gs['notif_app']) == 1,
              notifSms: _i(gs['notif_sms']) == 1,
              notifEmail: _i(gs['notif_email']) == 1,
              notifWhatsapp: _i(gs['notif_whatsapp']) == 1,
            ));
    if (res == null) return;
    setState(() => _busy = true);
    await ApiService.request('save_geofence_notif', {
      'device_id': widget.device.id,
      'geofence_id': _i(gs['geofence_id']),
      ...res
    });
    await _loadLinked();
    if (mounted) setState(() => _busy = false);
  ...
  Future<void> _linkExisting() async {
    ...
    final r = widget.device.userId != null
        ? await ApiService.request(
            'geofences', {'view_as': widget.device.userId})
    ...
    final available =
        all.where((g) => !linkedIds.contains(_i(g['id']))).toList();
    if (!mounted) return;
    if (available.isEmpty) {
      _snack(tr('gf_none_available'), const Color(0xFF1565C0));
      return;
    }
    final picked = await showDialog<int>(
        context: context,
        builder: (c) => AlertDialog(
                title: Text(tr('gf_link_existing'),
                    style: const TextStyle(
                        fontFamily: 'Cairo',
                        fontSize: 15,
                        fontWeight: FontWeight.w600)),
    ...
    setState(() => _busy = true);
    await ApiService.request('link_device_geofence',
        {'device_id': widget.device.id, 'geofence_id': picked});
    await _loadLinked();
    if (mounted) {
  ...
  Future<void> _createNew() async {
    final res = await Navigator.push<Map<String, dynamic>>(
        context,
        MaterialPageRoute(
            builder: (_) => _GeofenceDrawScreen(device: widget.device)));
    if (res == null) return;
    setState(() => _busy = true);
    final r = await ApiService.request('add_geofence',
        {'name': res['name'], 'area': res['area'], 'calendarId': 0});
    final gfId = _i(r['id']);
    if (gfId > 0) {
      await ApiService.request('link_device_geofence',
          {'device_id': widget.device.id, 'geofence_id': gfId});
      ...
      if (mounted) {
        setState(() => _busy = false);
        _snack(tr('gf_created'), const Color(0xFF6BA539));
        _maybePromptAlert();
      }
    } else {
      if (mounted) {
        setState(() => _busy = false);
        _snack(tr('gf_create_failed'), const Color(0xFFC41E3A));
      }
    }
  }
  ...
  void _maybePromptAlert() {
    ...
    showDialog(
        context: context,
        builder: (c) => AlertDialog(
                title: Text(tr('gf_enable_alert'),
                    style: const TextStyle(
                        fontFamily: 'Cairo',
                        fontSize: 15,
                        fontWeight: FontWeight.w600)),
                content: Text(tr('gf_enable_alert_q'),
                    style: const TextStyle(fontFamily: 'Cairo', fontSize: 13)),
                actions: [
                  TextButton(
                      onPressed: () => Navigator.pop(c),
                      child: Text(tr('gf_later'),
                          style: const TextStyle(fontFamily: 'Cairo'))),
                  TextButton(
                      onPressed: () {
                        Navigator.pop(c);
                        widget.onEnableAlert();
                      },
                      child: Text(tr('gf_enable'),
                          style: const TextStyle(
                              fontFamily: 'Cairo', color: Color(0xFF6BA539))))
                ]));
  }
      ...
      ListView(padding: const EdgeInsets.all(16), children: [
        if (!widget.alertEnabled)
          Container(
              ...
              child: Row(children: [
                const Icon(Icons.warning_amber_rounded,
                    color: Color(0xFF9A7B00), size: 20),
                const SizedBox(width: 8),
                Expanded(
                    child: Text(tr('gf_not_enabled'),
                        style: const TextStyle(
                            fontFamily: 'Cairo',
                            fontSize: 11,
                            color: Color(0xFF7A5C00)))),
                TextButton(
                    onPressed: widget.onEnableAlert,
                    style: TextButton.styleFrom(
        ...
        const SizedBox(height: 16),
        Text(tr('gf_linked_count', {'n': '${_linked.length}'}),
            style: const TextStyle(
                fontFamily: 'Cairo',
                fontSize: 13,
                fontWeight: FontWeight.w600)),
        ...
        if (_linked.isEmpty)
          ...
          ..._linked.map((gs) {
            final gfId = _i(gs['geofence_id']);
            final name = (gs['name']?.toString().trim().isNotEmpty == true)
                ? gs['name'].toString()
                : '${tr('tab_geofence')} $gfId';
            final notifApp = _i(gs['notif_app']) == 1;
            final notifSms = _i(gs['notif_sms']) == 1;
            final notifEmail = _i(gs['notif_email']) == 1;
            final notifWa = _i(gs['notif_whatsapp']) == 1;
            final evEnter = _i(gs['ev_enter']) == 1;
            final evExit = _i(gs['ev_exit']) == 1;
            final evSpeed = _i(gs['ev_speed']) == 1;
            final stopEng = _i(gs['stop_engine']) == 1;
            final chips = <Widget>[];
            if (evEnter)
              chips.add(_gchip(tr('gf_chip_enter'), const Color(0xFF2E7D32),
                  const Color(0xFFE8F5E9)));
            if (evExit)
              chips.add(_gchip(tr('gf_chip_exit'), const Color(0xFFC41E3A),
                  const Color(0xFFFFEBEE)));
            if (evSpeed)
              chips.add(_gchip(tr('gf_speed'), const Color(0xFFE65100),
                  const Color(0xFFFFF3E0)));
            if (stopEng)
              chips.add(_gchip(tr('gf_stop_engine'), const Color(0xFF6A1B9A),
                  const Color(0xFFF3E5F5)));
            if (notifApp)
              chips.add(_gchip(tr('gf_app'), const Color(0xFF1565C0),
                  const Color(0xFFE3F2FD)));
            if (notifEmail)
              chips.add(_gchip(tr('gf_email'), const Color(0xFFAD1457),
                  const Color(0xFFFCE4EC)));
            if (notifWa)
              chips.add(_gchip(tr('gf_whatsapp'), const Color(0xFF1B5E20),
                  const Color(0xFFE8F5E9)));
            if (notifSms)
              chips.add(_gchip(tr('gf_sms'), const Color(0xFF00695C),
                  const Color(0xFFE0F2F1)));
            if (chips.isEmpty)
              chips.add(_gchip(tr('gf_no_settings'), const Color(0xFF9E9E9E),
                          ...
                          Row(children: [
                            ...
                            IconButton(
                                icon: const Icon(Icons.tune,
                                    color: Color(0xFF1565C0), size: 20),
                                tooltip: tr('gf_settings_btn'),
                                onPressed:
                                    _busy ? null : () => _openSettings(gs),
                                visualDensity: VisualDensity.compact),
                            IconButton(
                                icon: const Icon(Icons.delete_outline,
                                    color: Color(0xFFC41E3A), size: 20),
                                onPressed:
                                    _busy ? null : () => _unlink(gfId, name),
                                visualDensity: VisualDensity.compact)
                          ]),
                          Padding(
...
class _GeofenceSettingsSheet extends StatefulWidget {
  ...
  final bool evEnter,
      ...
      notifEmail,
      notifWhatsapp;
  const _GeofenceSettingsSheet(
      {required this.name,
      required this.evEnter,
      required this.evExit,
      required this.evSpeed,
      required this.stopEngine,
      required this.notifApp,
      required this.notifSms,
      this.notifEmail = false,
      this.notifWhatsapp = false});
  @override
  State<_GeofenceSettingsSheet> createState() => _GeofenceSettingsSheetState();
}
...
class _GeofenceSettingsSheetState extends State<_GeofenceSettingsSheet> {
  ...
  void initState() {
    super.initState();
    _enter = widget.evEnter;
    _exit = widget.evExit;
    _speed = widget.evSpeed;
    _stop = widget.stopEngine;
    _app = widget.notifApp;
    _sms = widget.notifSms;
    _email = widget.notifEmail;
    _whatsapp = widget.notifWhatsapp;
  }
  ...
  Widget _chkRow(String l, String sub, bool v, ValueChanged<bool> cb,
          {Color? c}) =>
      InkWell(
          onTap: () => cb(!v),
          borderRadius: BorderRadius.circular(8),
          child: Padding(
              ...
              child: Row(children: [
                Checkbox(
                    value: v,
                    onChanged: (x) => cb(x!),
                    activeColor: c ?? const Color(0xFFC41E3A),
                    materialTapTargetSize: MaterialTapTargetSize.shrinkWrap),
                Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
      ...
      child: Container(
        decoration: BoxDecoration(
            ...
            borderRadius:
                const BorderRadius.vertical(top: Radius.circular(20))),
        padding: EdgeInsets.fromLTRB(
            18, 10, 18, 18 + MediaQuery.of(context).viewInsets.bottom),
        child: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Center(
                  child: Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(2)))),
              Row(children: [
                const Icon(Icons.tune, color: Color(0xFF1565C0), size: 20),
                const SizedBox(width: 8),
                Expanded(
                    child: Text('${tr('gf_settings')} — ${widget.name}',
                        style: const TextStyle(
                            fontFamily: 'Cairo',
                            fontSize: 15,
                            fontWeight: FontWeight.w700),
              ...
              _chkRow(tr('gf_enter'), tr('gf_enter_sub'), _enter,
                  (v) => setState(() => _enter = v),
                  c: const Color(0xFF2E7D32)),
              _chkRow(tr('gf_exit'), tr('gf_exit_sub'), _exit,
                  (v) => setState(() => _exit = v),
                  c: const Color(0xFFC41E3A)),
              _chkRow(tr('gf_speed'), tr('gf_speed_sub'), _speed,
                  (v) => setState(() => _speed = v),
                  c: const Color(0xFFE65100)),
              _section(tr('gf_on_violation')),
              _chkRow(tr('gf_stop_engine'), tr('gf_stop_sub'), _stop,
                  (v) => setState(() => _stop = v),
                  c: const Color(0xFF6A1B9A)),
              _section(tr('gf_notify_method')),
              _chkRow(tr('gf_app'), tr('gf_app_sub'), _app,
                  (v) => setState(() => _app = v),
                  c: const Color(0xFF1565C0)),
              _chkRow(tr('gf_email'), tr('gf_email_sub'), _email,
                  (v) => setState(() => _email = v),
                  c: const Color(0xFFAD1457)),
              _chkRow(tr('gf_whatsapp'), tr('gf_whatsapp_sub'), _whatsapp,
                  (v) => setState(() => _whatsapp = v),
                  c: const Color(0xFF25D366)),
              _chkRow(tr('gf_sms'), tr('gf_sms_sub'), _sms,
                  (v) => setState(() => _sms = v),
                  ...
                  child: ElevatedButton.icon(
                      onPressed: () {
                        Navigator.pop(context, {
                          ...
                          'ev_exit': _exit ? 1 : 0,
                          'ev_speed': _speed ? 1 : 0,
                          'stop_engine': _stop ? 1 : 0,
                          'notif_app': _app ? 1 : 0,
                          'notif_sms': _sms ? 1 : 0,
                          'notif_email': _email ? 1 : 0,
                          'notif_whatsapp': _whatsapp ? 1 : 0
                        });
                      },
...
class _GeofenceDrawScreen extends StatefulWidget {
  ...
  @override
  State<_GeofenceDrawScreen> createState() => _GeofenceDrawScreenState();
}
...
class _GeofenceDrawScreenState extends State<_GeofenceDrawScreen> {
  ...
  void initState() {
    super.initState();
    if (widget.device.lat != null &&
        widget.device.lng != null &&
        widget.device.lat != 0 &&
        widget.device.lng != 0)
      _center = LatLng(widget.device.lat!, widget.device.lng!);
  }
  ...
  void _undo() {
    if (_pts.isNotEmpty) setState(() => _pts.removeLast());
  }

  void _switchShape(_GfShape s) {
    setState(() {
      _shape = s;
    });
  }
  ...
  void _save() {
    ...
    if (name.isEmpty) {
      _err(tr('gf_enter_name'));
      return;
    }
    String area;
    if (_shape == _GfShape.circle) {
      if (_center == null) {
        _err(tr('gf_tap_center'));
        return;
      }
      // Traccar يفسّر WKT بترتيب خط العرض أولاً: CIRCLE (lat lng, radius)
      area =
          'CIRCLE (${_center!.latitude} ${_center!.longitude}, ${_radius.round()})';
      ...
      area =
          'POLYGON (($coords, ${_pts.first.latitude} ${_pts.first.longitude}))';
    } else {
      if (_pts.length < 2) {
        _err(tr('gf_line_min'));
        return;
  ...
  Set<Marker> _markers() {
    if (_shape == _GfShape.circle)
      return _center != null
          ? {Marker(markerId: const MarkerId('c'), position: _center!)}
          : {};
    return _pts
        .asMap()
        .entries
        .map((e) => Marker(markerId: MarkerId('p${e.key}'), position: e.value))
        .toSet();
  }

  Set<Circle> _circles() => (_shape == _GfShape.circle && _center != null)
      ? {
          Circle(
              circleId: const CircleId('gf'),
              center: _center!,
              radius: _radius,
              fillColor: const Color(0x336BA539),
              strokeColor: const Color(0xFF6BA539),
              strokeWidth: 2)
        }
      : {};
  Set<Polygon> _polygons() => (_shape == _GfShape.polygon && _pts.length >= 3)
      ? {
          Polygon(
              polygonId: const PolygonId('gf'),
              points: _pts,
              fillColor: const Color(0x336BA539),
              strokeColor: const Color(0xFF6BA539),
              strokeWidth: 2)
      ...
      : {};
  Set<Polyline> _polylines() => (_shape == _GfShape.line && _pts.length >= 2)
      ? {
          Polyline(
              polylineId: const PolylineId('gf'),
              points: _pts,
              color: const Color(0xFF6BA539),
              width: 4)
        }
      : {};
  Widget _shapeBtn(String l, IconData ic, _GfShape s) {
    final sel = _shape == s;
    return Expanded(
        child: GestureDetector(
            onTap: () => _switchShape(s),
            child: Container(
                margin: const EdgeInsets.symmetric(horizontal: 3),
                padding: const EdgeInsets.symmetric(vertical: 8),
                decoration: BoxDecoration(
                    color:
                        sel ? const Color(0xFF1565C0) : const Color(0xFFF0F2F6),
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(
                        color: sel
                            ? const Color(0xFF1565C0)
                ...
                child: Column(children: [
                  Icon(ic,
                      size: 20,
                      color: sel ? Colors.white : const Color(0xFF8892A4)),
                  const SizedBox(height: 2),
                  Text(l,
                      style: TextStyle(
                          fontFamily: 'Cairo',
  ...
  @override
  Widget build(BuildContext context) {
    final initial = _center ??
        (_pts.isNotEmpty ? _pts.first : const LatLng(30.0444, 31.2357));
    final hint = _shape == _GfShape.circle
        ? (_center == null ? tr('gf_tap_center2') : tr('gf_tap_center3'))
        : (_shape == _GfShape.polygon
            ? tr('gf_poly_hint')
            : tr('gf_line_hint'));
    return Directionality(
        textDirection: I18n.isAr ? TextDirection.rtl : TextDirection.ltr,
        child: Scaffold(
              ...
              actions: [
                if (_shape != _GfShape.circle)
                  IconButton(
                      ...
                      tooltip: tr('gf_undo'),
                      onPressed: _pts.isEmpty ? null : _undo),
              ]),
          body: Column(children: [
            Container(
                ...
                child: Row(children: [
                  _shapeBtn(tr('gf_shape_circle'), Icons.circle_outlined,
                      _GfShape.circle),
                  _shapeBtn(tr('gf_shape_polygon'), Icons.pentagon_outlined,
                      _GfShape.polygon),
                  _shapeBtn(tr('gf_shape_line'), Icons.timeline, _GfShape.line)
                ])),
            Expanded(
                child: Stack(children: [
              GoogleMap(
                  ...
                  child: Container(
                      ...
                      child: Text(hint,
                          ...
                          style: const TextStyle(
                              ...
                              fontSize: 12,
                              color: Colors.white)))),
              if (_shape != _GfShape.circle && _pts.isNotEmpty)
                Positioned(
                    bottom: 10,
                    right: 10,
                    child: Container(
                        ...
                        child: Text(tr('gf_points', {'n': '${_pts.length}'}),
                            style: const TextStyle(
                                fontFamily: 'Cairo',
                                fontSize: 12,
                                color: Colors.white,
            ...
            Container(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  if (_shape == _GfShape.circle) ...[
                    const SizedBox(height: 8),
                    Row(children: [
                      Text(tr('gf_radius', {'n': '${_radius.round()}'}),
                          style: const TextStyle(
                              fontFamily: 'Cairo',
                              fontSize: 12,
                              fontWeight: FontWeight.w600)),
