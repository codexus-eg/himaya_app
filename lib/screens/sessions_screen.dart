...
void openSessions(BuildContext context,
    {int? userId, String? subtitle, bool canRevoke = true}) {
  Navigator.of(context, rootNavigator: true).push(MaterialPageRoute(
    builder: (_) => SessionsScreen(
        userId: userId, subtitle: subtitle, canRevoke: canRevoke),
  ));
}
...
class SessionsScreen extends StatefulWidget {
  ...
  const SessionsScreen(
      {super.key, this.userId, this.subtitle, this.canRevoke = true});
...
class _SessionsScreenState extends State<SessionsScreen> {
  ...
  Future<void> _load([int attempt = 0]) async {
    ...
    if (raw is List) {
      ...
      setState(() {
        _loading = false;
        _error = r['error']?.toString() ?? tr('cl_fail');
      });
    }
  }
  ...
  Future<void> _revoke(Map s) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        title: Text(tr('ses_revoke_q'),
            style: cairo.copyWith(fontSize: 15, fontWeight: FontWeight.bold)),
        content: Text(s['name']?.toString() ?? '',
            style: cairo.copyWith(fontSize: 13)),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(c, false),
              child: Text(tr('cl_cancel'), style: cairo)),
          TextButton(
      ...
      child: Scaffold(
        ...
        body: _loading
            ...
            : _error != null
                ? Center(
                    child: Text(_error!,
                        style: cairo.copyWith(color: Colors.grey)))
                : _sessions.isEmpty
                    ? Center(
                        child: Text(tr('ses_none'),
                            style: cairo.copyWith(color: Colors.grey)))
                    : ListView(
                        ...
                        children: [
                          Padding(
                            ...
                            child: Text(
                                tr('ses_count', {'n': '${_sessions.length}'}),
                                style: cairo.copyWith(
                                ...
                                Expanded(
                                  child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                            '${tr('ses_last_use')}: ${_ago(s['last_used']?.toString())}',
                                            style: cairo.copyWith(
                                                fontSize: 11,
                                                color: Colors.grey)),
                                        if (ip.isNotEmpty)
