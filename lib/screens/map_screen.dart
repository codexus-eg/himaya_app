                ...
                children: [
                  ...
                  GridView.count(
                    ...
                    children: _visibleCommands.map((c) {
                      ...
                      return GestureDetector(
                        onTap: () {
                          setState(() { _selectedCmd = c['id'] as String; _result = null; });
                          // الأوامر التي تُظهر حقولًا (كلمة مرور / نص / أرقام SOS)
                          final needsFields = (c['isCustom'] == true) || (c['isSos'] == true) ||
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (_selectedCmd == 'sos') {
        ...
        _logCmd(sosCmd, cmdOk ? 'sent' : 'failed', cmdOk ? '' : (cmdRes['error']?.toString() ?? ''));
        ...
        if (!cmdOk || saveRes['success'] != true) {
          ...
          setState(() { _result = e ?? tr('res_sos_failed'); _loading = false; });
          return;
        }
...
class _DeviceAlertsScreenState extends State<DeviceAlertsScreen> {
  ...
  Future<void> _loadAlerts([int attempt = 0]) async {
    ...
    if (attempt == 0 && mounted) setState(() { _loading = true; _error = null; });
    String? err;
    try {
      ...
      if (r['success'] == true) {
        ...
        if (single) {
          for (final a in raw) { a['_dev'] = widget.device; }
          list.addAll(raw);
        } else {
          ...
          final byTid = <int, DeviceModel>{ for (final d in devs) d.traccarId: d };
          for (final a in raw) {
            final rv = a['deviceId'];
    ...
    setState(() { _error = err ?? tr('alerts_load_failed'); _loading = false; });
  }
            ...
            : _alerts.isEmpty
                ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(_error != null
                            ...
                            : (_alertsDisabled ? Icons.notifications_off_outlined : Icons.notifications_none_outlined),
                         size: 64, color: Colors.white24),
                    const SizedBox(height: 12),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 32),
                      child: Text(_error ?? (_alertsDisabled ? tr('alerts_off_title') : tr('alerts_empty')),
                           textAlign: TextAlign.center,
                    ...
                    if (_error == null && _alertsDisabled) Padding(
                      padding: const EdgeInsets.fromLTRB(32, 8, 32, 0),
                      child: Text(tr('alerts_off_hint'), textAlign: TextAlign.center,
                        style: const TextStyle(color: Colors.white38, fontFamily: 'Cairo', fontSize: 12, height: 1.5)),
                    ),
                    const SizedBox(height: 8),
                    TextButton.icon(onPressed: _loadAlerts, icon: const Icon(Icons.refresh, size: 16, color: Color(0xFFF59E0B)), label: Text(_error != null ? tr('retry') : tr('refresh'), style: const TextStyle(fontFamily: 'Cairo', color: Color(0xFFF59E0B)))),
                  ]))
                : RefreshIndicator(
