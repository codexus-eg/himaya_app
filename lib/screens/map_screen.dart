...
class _MapScreenState extends State<MapScreen> {
  ...
  bool _cameraMoving =
      false; // الكاميرا بتتحرك حاليًا (تفاعل مستخدم أو حركة برمجية)
  bool _progCam =
      false; // آخر حركة كاميرا كانت برمجية (متابعة) — مش تفاعل مستخدم
  DateTime?
      _lastCamGesture; // آخر مرة المستخدم حرّك/زوّم الخريطة بإيده (grace للتتبع)
  final Map<int, LatLng> _lastMarkerPos =
      {}; // آخر موقع معروض لكل جهاز (forward-only ضد التذبذب)
  ...
  Future<void> _showDeviceFromNotification() async {
    ...
    debugPrint(
        '[MAP] showDeviceFromNotif tid=$tid did=$did devices=${_devices.length}');
    ...
    if (_devices.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(tr('map_devices_not_loaded'),
    ...
    for (final d in _devices) {
      if (tid != null && (d.id == tid || d.traccarId == tid)) {
        target = d;
        break;
      }
    }
    debugPrint(
        '[MAP] target found=${target != null} name=${target?.name} ids=${_devices.map((d) => "${d.id}/${d.traccarId}").toList()}');

    if (target == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(tr('map_device_not_found', {'id': '$tid'}),
            style: const TextStyle(fontFamily: 'Cairo')),
        backgroundColor: const Color(0xFFC41E3A),
      ));
  ...
  void initState() {
    ...
    WidgetsBinding.instance
        .addPostFrameCallback((_) => _showDeviceFromNotification());
    ...
    _tickTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      ...
      if (widget.initialDevices != null || (_showPopup && _popupDevice != null))
        setState(() {});
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final bgRect = RRect.fromRectAndRadius(
          const Rect.fromLTWH(0, imgSize, totalWidth, labelHeight),
        ...
        tp.paint(
            canvas,
            Offset((totalWidth - tp.width) / 2,
                imgSize + (labelHeight - tp.height) / 2));
  ...
  Future<void> _buildMarkersAsync(List<DeviceModel> devices) async {
    ...
    for (final d in devices) {
      if (d.lat == null || d.lng == null) noPosOrder[d.id] = npIdx++;
    }
    ...
    final futures = devices.map((device) async {
      ...
      final speedLimit =
          device.speedLimit > 0 ? device.speedLimit.toDouble() : 100.0;
      ...
      } else if (device.isInactive)
        statusColor = const Color(0xFF9E9E9E);
      else if (isOverSpeed)
        statusColor = const Color(0xFFC41E3A);
      else if (device.isMoving)
        statusColor = const Color(0xFF6BA539);
      // محرك يعمل والمركبة واقفة ≥ دقيقتين (يحسبها السيرفر) — يحترم إعداد الحساب
      else if (device.idle && _idleOn)
      ...
      final cacheKey =
          '${vehicleType}_${statusColor.toARGB32()}_${isOffline ? 'off' : 'on'}${hasPosition ? '' : '_nopos'}';
      ...
      final int np = noPosOrder[device.id] ?? 0;
      LatLng pos = hasPosition
          ? LatLng(device.lat!, device.lng!)
          : LatLng(_noPosBaseLat + np * _noPosGapDeg,
              _noPosBaseLng + (np % 2) * _noPosGapDeg);
      ...
      if (hasPosition) {
        ...
        if (t != null &&
            prevT != null &&
            !t.isAfter(prevT) &&
            _lastMarkerPos.containsKey(device.id)) {
          pos = _lastMarkerPos[
              device.id]!; // الموقع الجديد أقدم → أبقِ على المعروض
        } else {
  ...
  static const Set<String> _spriteTypes = {
    'car',
    'sedan',
    'bus',
    'motorcycle',
  ...
  String _spriteState(Color color, bool isOffline, bool noPosition) {
    ...
    switch (color.toARGB32()) {
      case 0xFF6BA539:
        return 'green'; // moving
      case 0xFF2196F3:
        return 'blue'; // online/stopped
      case 0xFFFFC107:
        return 'yellow'; // محرك يعمل والمركبة واقفة
      case 0xFFC41E3A:
  ...
  static const Map<String, double> _spriteRatio = {
    'truck': 4.2,
    'bus': 4.7,
    'van': 4.7,
    'excavator': 3.6
  ...
  Future<BitmapDescriptor?> _spriteMarker(String type, String state) async {
    ...
    try {
      ...
      final icon = BitmapDescriptor.bytes(data.buffer.asUint8List(),
          imagePixelRatio: _spriteRatio[type] ?? 5.5);
      _iconCache[key] = icon;
      return icon;
    } catch (_) {
      return null;
  ...
  Future<BitmapDescriptor> _buildVehicleMarker(
    ...
    if (_spriteTypes.contains(norm)) {
      final sp =
          await _spriteMarker(norm, _spriteState(color, isOffline, noPosition));
      if (sp != null) return sp;
    }

    const iW = 64.0, iH = 110.0;
    const scale =
        4.0; // supersample → crisp marker on 3x/4x screens (same on-screen size via imagePixelRatio)
    final rec = ui.PictureRecorder();
    final c = Canvas(rec, const Rect.fromLTWH(0, 0, iW * scale, iH * scale));
    c.scale(scale); // كل الرسم يفضل بإحداثيات logical، الكانفاس بيكبّر للدقة

    // Draw vehicle shape (dimmed if offline or no position)
    final drawColor =
        (isOffline || noPosition) ? color.withValues(alpha: 0.45) : color;
    ...
    // Offline badge - red circle with X centered on car body
      ...
      const bX = iW / 2, bY = iH / 2;
      c.drawCircle(
          const Offset(bX, bY), bR, Paint()..color = const Color(0xCCEF5350));
      final xPaint = Paint()
        ..color = Colors.white
        ..strokeWidth = 2.5
        ..strokeCap = StrokeCap.round;
      c.drawLine(
          const Offset(bX - 7, bY - 7), const Offset(bX + 7, bY + 7), xPaint);
      c.drawLine(
          const Offset(bX + 7, bY - 7), const Offset(bX - 7, bY + 7), xPaint);
    }
  ...
  void _drawTopDown(Canvas c, String type, Color col, Rect r) {
    ...
    final glare = Paint()..color = Colors.white.withValues(alpha: 0.22);
    final border = Paint()
    ...
    final mirror = Paint()..color = col.withValues(alpha: 0.85);

    // Shadow
    c.drawOval(
        Rect.fromLTWH(x + w * 0.1, y + h * 0.88, w * 0.8, h * 0.12),
        Paint()
    ...
    switch (type) {
      // Sedan ??????????????????????????????????????????????????????
      case 'car':
        _topdownSedan(c, x, y, w, h, col, body, roof, glass, glare, border, hl,
            tl, mirror);
        break;

      // SUV (wider, taller roof) ???????????????????????????????????
      case 'suv':
        _topdownSUV(c, x, y, w, h, col, body, roof, glass, glare, border, hl,
            tl, mirror);
        break;

      // Pickup ?????????????????????????????????????????????????????
      case 'pickup':
        _topdownPickup(c, x, y, w, h, col, body, roof, glass, glare, border, hl,
            tl, mirror);
        break;
      ...
      case 'van':
        _topdownVan(
            c, x, y, w, h, col, body, roof, glass, glare, border, hl, tl);
        break;
  ...
  void _topdownSedan(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.06, y + h * 0.05, w * 0.88, h * 0.9),
            10,
            10),
        body);
    // Roof/cabin
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.14, y + h * 0.28, w * 0.72, h * 0.42),
            6,
            6),
        roof);
    // Front windshield
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.18, y + h * 0.28)
          ..lineTo(x + w * 0.82, y + h * 0.28)
          ..lineTo(x + w * 0.76, y + h * 0.12)
          ..lineTo(x + w * 0.24, y + h * 0.12)
          ..close(),
        glass);
    // Glare on windshield
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.22, y + h * 0.26)
          ..lineTo(x + w * 0.4, y + h * 0.26)
          ..lineTo(x + w * 0.36, y + h * 0.13)
          ..lineTo(x + w * 0.22, y + h * 0.13)
          ..close(),
        glare);
    // Rear windshield
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.2, y + h * 0.7)
          ..lineTo(x + w * 0.8, y + h * 0.7)
          ..lineTo(x + w * 0.76, y + h * 0.86)
          ..lineTo(x + w * 0.24, y + h * 0.86)
          ..close(),
        Paint()..color = const Color(0x9990CAF9));
    // Mirrors
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x, y + h * 0.22, w * 0.07, h * 0.06), 1, 1),
        mirror);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.93, y + h * 0.22, w * 0.07, h * 0.06),
            1,
            1),
        mirror);
    // Headlights
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.05, w * 0.28, h * 0.05), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.62, y + h * 0.05, w * 0.28, h * 0.05),
            1,
            1),
        hl);
    // Tail lights
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.9, w * 0.28, h * 0.05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.62, y + h * 0.9, w * 0.28, h * 0.05), 1, 1),
        tl);
    // Outline
    c.drawRRect(
        RRect.fromRectXY(
  ...
  void _topdownSUV(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.04, y + h * 0.04, w * 0.92, h * 0.92),
            9,
            9),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.26, w * 0.8, h * 0.48), 6, 6),
        roof);
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.14, y + h * 0.26)
          ..lineTo(x + w * 0.86, y + h * 0.26)
          ..lineTo(x + w * 0.82, y + h * 0.1)
          ..lineTo(x + w * 0.18, y + h * 0.1)
          ..close(),
        glass);
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.2, y + h * 0.24)
          ..lineTo(x + w * 0.38, y + h * 0.24)
          ..lineTo(x + w * 0.35, y + h * 0.11)
          ..lineTo(x + w * 0.2, y + h * 0.11)
          ..close(),
        glare);
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.14, y + h * 0.74)
          ..lineTo(x + w * 0.86, y + h * 0.74)
          ..lineTo(x + w * 0.82, y + h * 0.9)
          ..lineTo(x + w * 0.18, y + h * 0.9)
          ..close(),
        Paint()..color = const Color(0x9990CAF9));
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x, y + h * 0.2, w * 0.05, h * 0.08), 1, 1),
        mirror);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.95, y + h * 0.2, w * 0.05, h * 0.08), 1, 1),
        mirror);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.08, y + h * 0.04, w * 0.3, h * 0.05), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.62, y + h * 0.04, w * 0.3, h * 0.05), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.08, y + h * 0.91, w * 0.3, h * 0.05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.62, y + h * 0.91, w * 0.3, h * 0.05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.04, y + h * 0.04, w * 0.92, h * 0.92),
  ...
  void _topdownPickup(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.06, y + h * 0.05, w * 0.88, h * 0.9), 8, 8),
        body);
    // Cab roof (front 55%)
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.12, y + h * 0.26, w * 0.76, h * 0.28),
            5,
            5),
        roof);
    // Windshield
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.16, y + h * 0.26)
          ..lineTo(x + w * 0.84, y + h * 0.26)
          ..lineTo(x + w * 0.8, y + h * 0.12)
          ..lineTo(x + w * 0.2, y + h * 0.12)
          ..close(),
        glass);
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.2, y + h * 0.24)
          ..lineTo(x + w * 0.38, y + h * 0.24)
          ..lineTo(x + w * 0.35, y + h * 0.13)
          ..lineTo(x + w * 0.2, y + h * 0.13)
          ..close(),
        glare);
    // Bed divider line
    c.drawLine(
        Offset(x + w * 0.08, y + h * 0.56),
        Offset(x + w * 0.92, y + h * 0.56),
        Paint()
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x, y + h * 0.2, w * 0.07, h * 0.06), 1, 1),
        mirror);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.93, y + h * 0.2, w * 0.07, h * 0.06), 1, 1),
        mirror);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.05, w * 0.28, h * 0.05), 1, 1),
  ...
  void _topdownVan(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.08, y + h * 0.25, w * 0.84, h * 0.5), 5, 5),
        roof);
    // Front window (wider for van)
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.1, y + h * 0.25)
          ..lineTo(x + w * 0.9, y + h * 0.25)
          ..lineTo(x + w * 0.88, y + h * 0.1)
          ..lineTo(x + w * 0.12, y + h * 0.1)
          ..close(),
        glass);
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.14, y + h * 0.23)
          ..lineTo(x + w * 0.4, y + h * 0.23)
          ..lineTo(x + w * 0.38, y + h * 0.11)
          ..lineTo(x + w * 0.14, y + h * 0.11)
          ..close(),
        glare);
    // Rear window
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.1, y + h * 0.75)
          ..lineTo(x + w * 0.9, y + h * 0.75)
          ..lineTo(x + w * 0.88, y + h * 0.9)
          ..lineTo(x + w * 0.12, y + h * 0.9)
          ..close(),
        Paint()..color = const Color(0x7790CAF9));
    // Side sliding door line
    c.drawLine(
        Offset(x + w * 0.06, y + h * 0.42),
        Offset(x + w * 0.94, y + h * 0.42),
        Paint()
  ...
  void _topdownBus(Canvas c, double x, double y, double w, double h, Color col,
      Paint body, Paint roof, Paint glass, Paint border, Paint hl, Paint tl) {
    // Tall rectangle
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.04, y + h * 0.02, w * 0.92, h * 0.96),
            7,
            7),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.2, w * 0.8, h * 0.6), 4, 4),
        roof);
    // Front windshield
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.05, w * 0.8, h * 0.13), 3, 3),
        glass);
    // Rear window
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.82, w * 0.8, h * 0.13), 3, 3),
        Paint()..color = const Color(0x7790CAF9));
    // 3 top windows
    for (int i = 0; i < 3; i++) {
      c.drawRRect(
          RRect.fromRectXY(
              Rect.fromLTWH(x + w * 0.12 + i * w * 0.28, y + h * 0.22, w * 0.22,
                  h * 0.14),
              2,
              2),
          glass);
    ...
    for (int i = 0; i < 3; i++) {
      c.drawRRect(
          RRect.fromRectXY(
              Rect.fromLTWH(x + w * 0.12 + i * w * 0.28, y + h * 0.64, w * 0.22,
                  h * 0.14),
              2,
              2),
          glass);
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.08, y + h * 0.02, w * 0.3, h * 0.04), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.62, y + h * 0.02, w * 0.3, h * 0.04), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.08, y + h * 0.94, w * 0.3, h * 0.04), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.62, y + h * 0.94, w * 0.3, h * 0.04), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.04, y + h * 0.02, w * 0.92, h * 0.96),
  ...
  void _topdownTruck(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.06, y + h * 0.38, w * 0.88, h * 0.58),
            5,
            5),
        body);
    // Cargo roof lines
    c.drawLine(
        ...
        Offset(x + w * 0.94, y + h * 0.55),
        Paint()
          ..color = Colors.black.withValues(alpha: 0.12)
          ..strokeWidth = 1);
    c.drawLine(
        Offset(x + w * 0.06, y + h * 0.72),
        Offset(x + w * 0.94, y + h * 0.72),
        Paint()
          ..color = Colors.black.withValues(alpha: 0.12)
          ..strokeWidth = 1);
    // Cab (front 40%)
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.06, y + h * 0.04, w * 0.88, h * 0.36),
            8,
            8),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.12, y + h * 0.2, w * 0.76, h * 0.18), 4, 4),
        roof);
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.14, y + h * 0.2)
          ..lineTo(x + w * 0.86, y + h * 0.2)
          ..lineTo(x + w * 0.82, y + h * 0.07)
          ..lineTo(x + w * 0.18, y + h * 0.07)
          ..close(),
        glass);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.04, w * 0.28, h * 0.04), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.62, y + h * 0.04, w * 0.28, h * 0.04),
            1,
            1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.92, w * 0.28, h * 0.04), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.62, y + h * 0.92, w * 0.28, h * 0.04),
            1,
            1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(
                x + w * 0.06, y + h * 0.04, w * 0.88, h * 0.58 + 0.34),
            5,
            5),
        border);
  }

  void _topdownMoto(Canvas c, double x, double y, double w, double h, Color col,
      Paint body, Paint border) {
    // Tank/frame - narrow elongated
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.3, y + h * 0.06, w * 0.4, h * 0.7), 8, 8),
        body);
    // Handlebars
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.1, y + h * 0.08, w * 0.8, h * 0.08), 3, 3),
        body);
    // Seat
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.32, y + h * 0.42, w * 0.36, h * 0.28),
            5,
            5),
  ...
  void _topdownTukTuk(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.18, y + h * 0.24, w * 0.64, h * 0.44),
            6,
            6),
        roof);
    ...
    c.drawPath(
        Path()
          ..moveTo(x + w * 0.22, y + h * 0.24)
          ..lineTo(x + w * 0.78, y + h * 0.24)
          ..lineTo(x + w * 0.68, y + h * 0.1)
          ..lineTo(x + w * 0.32, y + h * 0.1)
          ..close(),
        glass);
    // Rear open sides (lines)
    c.drawLine(
        Offset(x + w * 0.14, y + h * 0.7),
        Offset(x + w * 0.86, y + h * 0.7),
        Paint()
          ..color = Colors.black.withValues(alpha: 0.2)
          ..strokeWidth = 1.2);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.38, y + h * 0.04, w * 0.24, h * 0.06),
            2,
            2),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.2, y + h * 0.88, w * 0.22, h * 0.05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.58, y + h * 0.88, w * 0.22, h * 0.05),
            1,
            1),
        tl);
  ...
  void _topdownTractor(Canvas c, double x, double y, double w, double h,
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.28, y + h * 0.04, w * 0.44, h * 0.32),
            5,
            5),
        body);
    // Cabin (middle)
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.2, y + h * 0.3, w * 0.6, h * 0.28), 6, 6),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.26, y + h * 0.34, w * 0.48, h * 0.18),
            4,
            4),
        roof);
    // Rear axle body
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.22, y + h * 0.55, w * 0.56, h * 0.36),
            6,
            6),
        body);
    // Exhaust pipe
    c.drawLine(
        Offset(x + w * 0.42, y + h * 0.04),
        Offset(x + w * 0.42, y),
        Paint()
          ..color = col
          ..strokeWidth = w * 0.06
  ...
  void _topdownBoat(Canvas c, double x, double y, double w, double h, Color col,
      Paint body, Paint glass, Paint border) {
    // Hull teardrop
    final hull = Path()
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.2, y + h * 0.12, w * 0.6, h * 0.2), 5, 5),
        glass);
    // Cabin line
    c.drawLine(
        Offset(x + w * 0.1, y + h * 0.38),
        Offset(x + w * 0.9, y + h * 0.38),
        Paint()
          ..color = Colors.black.withValues(alpha: 0.2)
          ..strokeWidth = 1.2);
    // Motor hint
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * 0.38, y + h * 0.86, w * 0.24, h * 0.08),
            3,
            3),
  ...
  void _wheels4(Canvas c, double x, double y, double w, double h, Color col,
      Paint stroke) {
  ...
  void _smoothZoomTo(LatLng target, {double targetZoom = 16}) {
    ...
    ctrl.getZoomLevel().then((startZoom) {
      ...
      _zoomAnimTimer = Timer.periodic(const Duration(milliseconds: 48), (t) {
        i++;
        if (!mounted || _mapController == null || i > steps) {
          t.cancel();
          return;
        }
  ...
  void _flyBetween(LatLng from, LatLng to, {double targetZoom = 16}) {
    ...
    ctrl.getZoomLevel().then((startZoom) {
      ...
      final dist =
          math.sqrt(dLat * dLat + dLng * dLng); // مسافة بالدرجات (تقريبية)
      final dip =
          dist > 0.3 ? 3.0 : (dist > 0.03 ? 2.0 : 0.0); // زوم أوت حسب البُعد
      const steps = 24;
      int i = 0;
      _zoomAnimTimer = Timer.periodic(const Duration(milliseconds: 40), (t) {
        ...
        final z = zBase -
            dip * math.sin(p * math.pi); // منحنى: 0 عند الطرفين، أقصى في النص
        _mapController!
            .moveCamera(CameraUpdate.newLatLngZoom(LatLng(lat, lng), z));
      });
    });
  }
  ...
  void _recenterAfterResume() {
    if (_showPopup &&
        _popupDevice != null &&
        _popupDevice!.lat != null &&
        _popupDevice!.lng != null) {
      // يتبع موقع الماركر المرسوم فعلاً (_dispPos) مش الخام — يضمن الكاميرا على الجديد
      _mapController
          ?.animateCamera(CameraUpdate.newLatLng(_dispPos(_popupDevice!)));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final countdownJustReset =
        _lastRefreshCountdown <= 1 && provider.refreshCountdown > 7;
    ...
    if (widget.initialDevices == null &&
        isCurrentRoute &&
        (provider.devices != _lastDevices ||
            provider.inventory != _lastDevices ||
            countdownJustReset)) {
      final freshList = (provider.currentUser?.isDealer == true ||
              provider.currentUser?.isSubDealer == true)
          ? provider.inventory
          : provider.devices;
      ...
      if (_showPopup && _popupDevice != null) {
        ...
        if (match.isNotEmpty && !identical(match.first, _popupDevice)) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            ...
            match.first.lat != null &&
            match.first.lng != null) {
    ...
    if (widget.initialDevices != null &&
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _refreshScopedDevices();
      });
    }
      ...
      children: [
        ...
        GoogleMap(
          ...
          onMapCreated: (ctrl) {
            ...
            Future.delayed(
                const Duration(milliseconds: 500), _fitBoundsToDevices);
          },
              ...
              setState(() {
                ...
                if (_measurePoints.length >= 2) {
                  ...
                  for (int i = 1; i < _measurePoints.length; i++) {
                    ...
                    final x = math.sin(lat / 2) * math.sin(lat / 2) +
                  ...
                  _polylines
                      .removeWhere((p) => p.polylineId.value == 'measure');
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(
                        tr('map_distance', {'d': total.toStringAsFixed(2)}),
                        style: const TextStyle(fontFamily: 'Cairo')),
        ...
        Positioned(
          ...
          left: 10,
          child: _MapBtn(
              icon: Icons.zoom_out_map, onTap: () => _fitBoundsToDevices()),
        ),
        ...
        if (_devices.isNotEmpty && !_showPlayback)
          Positioned(
            ...
            child: Row(
              ...
              children: [
                _NavArrow(
                    icon: Icons.chevron_right, onTap: () => _navigateDev(-1)),
                ...
                const SizedBox(width: 8),
                _NavArrow(
                    icon: Icons.chevron_left, onTap: () => _navigateDev(1)),
              ],
            ),
          ),
      ...
      onTap: () async {
        ...
        if (widget.initialDevices != null) {
          ...
          final scoped = [...provider.devices, ...provider.inventory]
              .where((d) => d.userId == widget.viewAsUserId)
              .toList();
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          hintStyle: const TextStyle(
              color: Color(0xFF8892A4), fontSize: 11, fontFamily: 'Cairo'),
          contentPadding:
          ...
          _MapBtn(
              ...
              onTap: () {
                ...
                if (_showPopup && _popupDevice != null) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(tr('map_select_first'),
                        style: const TextStyle(fontFamily: 'Cairo')),
  ...
  Widget _buildPlaybackControls() {
    final p =
        _playbackPositions.isNotEmpty ? _playbackPositions[_playbackIdx] : null;
    final spd =
        p != null ? ((p['speed'] ?? 0) * 1.852).toStringAsFixed(1) : '0';
    final t = p?['fixTime'] != null
        ? _formatTime(DateTime.tryParse(p!['fixTime']))
        : '';
  ...
  void _navigateDev(int dir) {
    ...
    int cur =
        _popupDevice != null ? _devices.indexOf(_popupDevice!) : _selectedIndex;
    ...
    _buildMarkers(
        _devices); // أعد البناء ليطفو ماركر الجهاز المختار فوق المتراكبين
  ...
  Future<void> _maybeRecenter(double lat, double lng) async {
    ...
    if (_lastCamGesture != null &&
        DateTime.now().difference(_lastCamGesture!) <
            const Duration(seconds: 4)) return;
    try {
  ...
  IconData _batteryIcon(String? batStr) {
    final pct =
        int.tryParse(batStr?.replaceAll(RegExp(r'[^0-9]'), '') ?? '') ?? 0;
  ...
  String _formatTime(DateTime? dt) {
    ...
    if (diff.inMinutes < 60)
      return tr('t_min_sec',
          {'m': '${diff.inMinutes}', 's': '${diff.inSeconds % 60}'});
    if (diff.inHours < 24)
      return tr(
          't_hr_min', {'h': '${diff.inHours}', 'm': '${diff.inMinutes % 60}'});
  ...
  Widget _buildDevicePopup(DeviceModel device) {
    ...
    final liveLastSeen = liveDev?.lastSeen ??
        liveDev?.lastUpdate ??
        device.lastSeen ??
        device.lastUpdate;
    ...
    if (widget.viewAsUserId == null && liveDev != null) device = liveDev;

    final dotColor = device.isInactive
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: const [
          BoxShadow(
              color: Color(0x1F000000), blurRadius: 14, offset: Offset(0, 4))
        ],
      ),
        ...
        children: [
              ...
              Row(
                ...
                children: [
                  Column(
                    ...
                    children: [
                      Text('${device.deviceType} - ${device.icon}',
                          style: const TextStyle(
                              fontSize: 10,
                  ...
                  GestureDetector(
                    ...
                    child: const Icon(Icons.close,
                        size: 18, color: Color(0xFF8892A4)),
                  ),
                ],
              ),
            ],
          ...
          Builder(builder: (_) {
            ...
            if (!gps103) {
              boxes.add(_InfoBox(
                  label: tr('ib_battery'),
                  value: device.batteryDisplay,
                  valueColor: const Color(0xFF6BA539),
                  icon: _batteryIcon(device.batteryDisplay),
                  iconColor: const Color(0xFF6BA539)));
            }
            if (!gps103) {
              boxes.add(_InfoBox(
                  label: tr('ib_power'),
                  value: (device.powerConnected || device.charge)
                      ? tr('ib_connected')
                      : tr('ib_disconnected'),
                  icon: (device.powerConnected || device.charge)
                      ? Icons.power
                      : Icons.power_off,
                  iconColor: (device.powerConnected || device.charge)
                      ? const Color(0xFF6BA539)
                      : const Color(0xFF8892A4)));
            }
            // المربع يظهر دائمًا: الجهاز الذي لم ترد منه إشارة ACC بعد يعرضه السيرفر
            // «مغلق منذ أول اتصال»، لأن غياب الإشارة لا يعني أن السلك غير موصول —
            // TK303 يرسل ACC عند تغيّرها فقط، فالمركبة التي لم تشتغل لا ترسل شيئًا.
            boxes.add(_InfoBox(
                label: tr('ib_engine'),
                value: ignText,
                dotColor: device.ignition == true
                    ? Colors.blue
                    : const Color(0xFF8892A4)));
          ...
          if (device.lat != null && device.lng != null) ...[
            ...
            Row(children: [
              const Icon(Icons.location_on, size: 10, color: Color(0xFF8892A4)),
              const SizedBox(width: 3),
              Expanded(
                  child: _AsyncAddressText(
                key: ValueKey('addr_${device.id}'),
                lat: device.lat!,
                lng: device.lng!,
                style: const TextStyle(
                    fontSize: 9, color: Color(0xFF8892A4), fontFamily: 'Cairo'),
              )),
              if (liveLastSeen != null)
                Text(
                  tr('dev_last_update', {'t': _formatTime(liveLastSeen)}),
                  style: const TextStyle(
                      fontSize: 9,
                      color: Color(0xFF8892A4),
                      fontFamily: 'Cairo'),
            ...
            children: [
              ...
              Expanded(
                child: OutlinedButton(
                  ...
                  child: Text(tr('btn_reports'),
                      style: TextStyle(
                          fontSize: 11, fontFamily: 'Cairo', color: onSurface)),
                ),
              ),
              if (!context.read<AppProvider>().isViewOnly) ...[
                ...
                Expanded(
                  child: OutlinedButton(
                    ...
                    child: Text(tr('btn_send_command'),
                        style: TextStyle(
                            fontSize: 9,
                            fontFamily: 'Cairo',
                            color: onSurface)),
    ...
    try {
      ...
      if (!serviceEnabled) {
        ...
        if (!serviceEnabled) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(tr('loc_enable_gps'),
            ...
            children: _vehicleIcons.map((v) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: isSelected
                        ? const Color(0xFFC41E3A).withValues(alpha: 0.15)
  ...
  // Map type cycling
  int _mapTypeIndex = 0;
  // القمر الصناعي وحده بلا أسماء شوارع فلا فائدة عملية منه — والهجين يغنيه
  // (نفس الصور + الأسماء). فاكتفينا بالعادية والهجين.
  final _mapTypes = [MapType.normal, MapType.hybrid];
  final _mapTypeLabels = ['mt_map', 'mt_hybrid'];
            ...
            child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    ...
                    const SizedBox(width: 8),
                    Expanded(
                        child: Text(tr('rv_title', {'name': device.name}),
                            style: const TextStyle(
                                color: Colors.white,
                                fontFamily: 'Cairo',
                                fontSize: 14,
                      ...
                      child: Container(
                        ...
                        child: Row(children: [
                          const Icon(Icons.calendar_today,
                              color: Color(0xFF6BA539), size: 16),
                          const SizedBox(width: 8),
                          Text(tr('lbl_from'),
                              style: const TextStyle(
                                  color: Colors.white54,
                      ...
                      if (distLoading)
                        const SizedBox(
                            ...
                            child: CircularProgressIndicator(
                                strokeWidth: 2, color: Color(0xFF6BA539)))
                      else
                      ...
                      onPressed: () {
                        ...
                        if (selectedPeriod == 'custom') {
                          if (!customTo.isAfter(customFrom)) {
                            ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
                                content: Text(tr('rv_invalid_range'),
                                    style:
                          ...
                          if (customTo.difference(customFrom).inDays > 30) {
                            ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
                                content: Text(tr('rv_max_30_days'),
                                    style:
                                        const TextStyle(fontFamily: 'Cairo')),
                                backgroundColor: const Color(0xFFC41E3A)));
                            return;
                        ...
                        Navigator.of(context, rootNavigator: true)
                          ...
                          builder: (_) => _RoutePlaybackScreen(
                            ...
                            speedLimit: lim,
                            customFrom:
                                selectedPeriod == 'custom' ? customFrom : null,
                            customTo:
                                selectedPeriod == 'custom' ? customTo : null,
                          ),
                        ));
                      },
                    ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color:
                  active ? const Color(0xFF2196F3) : const Color(0xFFE8EAEF)),
          boxShadow: const [BoxShadow(color: Color(0x14000000), blurRadius: 5)],
        ),
        child: Icon(icon,
            size: 16, color: active ? Colors.white : const Color(0xFF1A1F2E)),
      ),
    );
  }
}
        ...
        child: Row(children: [
          Container(
            ...
            child: Icon(widget.icon,
                size: 16,
                color: _enabled ? const Color(0xFFF59E0B) : Colors.white38),
          ),
          const SizedBox(width: 12),
          Expanded(
              child: Text(widget.label,
                  style: TextStyle(
                      color: _enabled ? Colors.white : Colors.white38,
                      fontFamily: 'Cairo',
                      fontSize: 13))),
          Switch(
            value: _enabled,
          ...
          children: [
            ...
            Row(
              children: [
                if (icon != null) ...[
                  Icon(icon,
                      size: 12, color: iconColor ?? const Color(0xFF8892A4)),
                  const SizedBox(width: 3),
                ],
                Flexible(
                  child: Text(
...
class _CommandModalState extends State<_CommandModal> {
  ...
  // اختيار أمر يحتاج كلمة مرور أو نصًّا يُظهر حقولًا أسفل الشاشة، وكان المستخدم
  // يسحب بيده ليصل إليها. ننزل تلقائيًا بعد اكتمال الرسم.
  /// [delays] تُمرَّر أطول عند الضغط على حقل: لوحة المفاتيح تستغرق ~300ms لتظهر،
  /// وقبل ظهورها يكون maxScrollExtent أقصر فلا يصل التمرير إلى الحقل فعليًا.
  void _scrollToFields([List<int> delays = const [80, 260]]) {
  ...
  Future<void> _loadSavedSos() async {
    try {
      final result =
          await ApiService.request('get_sos', {'deviceId': widget.device.id});
  ...
  static const _commands = [
    ...
    {
      'id': 'engineStop',
      'label': 'cmd_engine_stop',
      'desc': 'cmd_engine_stop_d',
    ...
    {
      'id': 'engineResume',
      'label': 'cmd_engine_resume',
      'desc': 'cmd_engine_resume_d',
  ...
  void _logCmd(String cmdText, String status, String result) {
    ApiService.logCommand(
            deviceId: widget.device.id,
            cmdType: _selectedCmd,
            cmdText: cmdText,
            status: status,
            result: result)
        .catchError((_) => <String, dynamic>{});
  ...
  Widget build(BuildContext context) {
    final cmd = _current;
    // التمرير مربوط بارتفاع لوحة المفاتيح الفعلي لا بتأخير مُخمَّن (كان 80/260ms
    // فيقع قبل ظهورها أصلًا). مع كل إطار من فتحها يرتفع المحتوى بالقدر نفسه
    // فيبدو ملتصقًا بحركتها؛ ولهذا jumpTo لا animateTo — الأنيميشن هو أنيميشنها.
    final kbInset = MediaQuery.of(context).viewInsets.bottom;
    if (kbInset > _lastKbInset) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted || !_scrollCtrl.hasClients) return;
        ...
        children: [
            ...
            child: Row(
              ...
              children: [
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(tr('btn_send_command'),
                      style: const TextStyle(
                          color: Colors.white,
                          fontSize: 13,
                          fontWeight: FontWeight.w600,
                          fontFamily: 'Cairo')),
                  Text('${widget.device.name} - ${widget.device.deviceType}',
                      style: const TextStyle(
                          color: Color(0xFF8892A4),
                          fontSize: 10,
                          fontFamily: 'Cairo')),
                ...
                Row(mainAxisSize: MainAxisSize.min, children: [
                  GestureDetector(
                      onTap: () => Navigator.of(context).push(MaterialPageRoute(
                          builder: (_) =>
                              CommandHistoryScreen(device: widget.device))),
                      child: Container(
                          width: 26,
                          height: 26,
                          decoration: const BoxDecoration(
                              color: Color(0x332196F3), shape: BoxShape.circle),
                  ...
                  GestureDetector(
                      ...
                      child: Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              shape: BoxShape.circle),
                          child: const Icon(Icons.close,
                              color: Color(0xFF8892A4), size: 14))),
                ]),
              ],
            ),
          ),
          Flexible(
            child: SingleChildScrollView(
              ...
              padding: EdgeInsets.fromLTRB(
                  14, 0, 14, 16 + MediaQuery.of(context).viewInsets.bottom),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(tr('cmd_choose'),
                  ...
                  GridView.count(
                    ...
                    children: _visibleCommands.map((c) {
                      ...
                      return GestureDetector(
                        onTap: () {
                          setState(() {
                            _selectedCmd = c['id'] as String;
                            _result = null;
                          });
                          // الأوامر التي تُظهر حقولًا (كلمة مرور / نص / أرقام SOS)
                          final needsFields = (c['isCustom'] == true) ||
                              (c['isSos'] == true) ||
                              (_cmdPwOn && c['needsPw'] == true);
                          if (needsFields) _scrollToFields();
                        },
                        child: Container(
                          padding: const EdgeInsets.symmetric(
                            ...
                            children: [
                              ...
                              Expanded(
                                  child: Column(
                                      ...
                                      children: [
                                    Text(tr(c['label'] as String),
                                        style: const TextStyle(
                                            color: Color(0xFFE8EAF0),
                                            fontSize: 10,
                                            fontWeight: FontWeight.w500,
                                            fontFamily: 'Cairo')),
                                    Text(tr(c['desc'] as String),
                                        style: const TextStyle(
                  ...
                  if (cmd['isCustom'] == true) ...[
                    ...
                    _DarkInput(
                        ...
                        hint: tr('cmd_custom_hint'),
                        textDirection: TextDirection.ltr),
                  ],
                  if (cmd['isSos'] == true) ...[
                    const SizedBox(height: 8),
                    Text(tr('cmd_emergency'),
                        style: const TextStyle(
                            color: Color(0x60FFFFFF),
                            fontSize: 9,
                            fontFamily: 'Cairo')),
                  ...
                  if (cmd['needsPw'] == true && _cmdPwOn) ...[
                    ...
                    Container(
                      padding: const EdgeInsets.all(10),
                      decoration: BoxDecoration(
                        color: const Color(0xFFC41E3A).withValues(alpha: 0.08),
                        borderRadius: BorderRadius.circular(10),
                        border: Border.all(
                  ...
                  if (_result != null) ...[
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: const Color(0xFFA5F3FC).withValues(alpha: 0.07),
                        borderRadius: BorderRadius.circular(8),
                        border: Border.all(
                            color:
                                const Color(0xFFA5F3FC).withValues(alpha: 0.2)),
                      ),
                      child: Text(_result!,
                          style: const TextStyle(
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (_selectedCmd == 'sos') {
        ...
        final cmdRes = await ApiService.sendCommand(
          deviceId: widget.device.id,
          type: 'custom',
          customText: sosCmd,
        );
        final cmdOk = cmdRes['success'] == true;
        // يُسجَّل كبقية الأوامر: مساره كان منفصلًا فلم يكن لأعطاله أي أثر يُفحص
        _logCmd(sosCmd, cmdOk ? 'sent' : 'failed',
            cmdOk ? '' : (cmdRes['error']?.toString() ?? ''));
        // حفظ الأرقام في قاعدة البيانات
        final saveRes = await ApiService.request('save_sos', {
          'deviceId': widget.device.id,
          'phone1': n1,
          'phone2': _sos2.text.trim(),
          'phone3': _sos3.text.trim(),
        });
        // ميزة أمان: ممنوع إعلان الحفظ ما لم ينجح الأمر والتخزين معًا
        if (!cmdOk || saveRes['success'] != true) {
          final e = (cmdOk ? saveRes['error'] : cmdRes['error'])?.toString();
          setState(() {
            _result = e ?? tr('res_sos_failed');
            _loading = false;
          });
          return;
        }
        setState(() {
          _result = tr('res_sos_saved');
          _loading = false;
        });
        return;
      }
      ...
      final needsPw = _cmdPwOn &&
          ['engineStop', 'engineResume', 'factoryReset', 'custom']
              .contains(_selectedCmd);
      if (needsPw) {
        if (_pwCtrl.text.trim().isEmpty) {
          setState(() {
            _result = tr('res_enter_pw');
            _loading = false;
          });
          return;
        }
      ...
      Future<List<dynamic>> fetchEvents() async {
        ...
        final from =
            now.subtract(const Duration(seconds: 120)).toIso8601String();
        ...
        final evResult = await ApiService.request('get_events', {
          ...
          'nodedup':
              1, // لا تدمج ردود الأوامر المتتالية (نفس النوع commandResult)
        });
        return List<dynamic>.from(evResult['data'] ??
            evResult['events'] ??
            (evResult is List ? evResult : []));
      }

      // ⚠️ خط الأساس يُلتقط قبل الإرسال — لتفادي ابتلاع رد سريع من الجهاز (1-2ث) ضمن خط الأساس
      final Set<String> baselineKeys =
          needsReply ? (await fetchEvents()).map(evKey).toSet() : <String>{};
      ...
      if (needsReply) {
        ...
        while (DateTime.now().isBefore(deadline)) {
          ...
          for (final e in events) {
            if (!baselineKeys.contains(evKey(e))) {
              fresh = e;
              break;
            }
          }
          if (fresh != null) {
            raw =
                fresh['attributes']?['result'] ?? fresh['attributes']?['data'];
            gotReply = true;
            break;
          }
        }
        ...
        if (gotReply) {
          ...
          if (_selectedCmd == 'engineStop' || _selectedCmd == 'engineResume') {
            ...
            if (rawUp.contains('RELAY 1')) {
              ...
              shown = _selectedCmd == 'engineStop'
                  ? tr('res_engine_stopped')
                  : tr('res_engine_resumed');
            }
          } else {
...
class _CommandHistoryScreenState extends State<CommandHistoryScreen> {
  ...
  String _cmdLabel(String t) {
    switch (t) {
      case 'deviceStatus':
        return I18n.isAr ? 'فحص الحالة' : 'Status check';
      case 'getParams':
        return I18n.isAr ? 'عرض الإعدادات' : 'Get parameters';
      case 'getVersion':
        return I18n.isAr ? 'نسخة الإصدار' : 'Version';
      case 'setLocation':
        return I18n.isAr ? 'إعدادات تحديد الموقع' : 'Location settings';
      case 'factoryReset':
        return I18n.isAr ? 'إعادة الضبط' : 'Factory reset';
      case 'engineStop':
        return I18n.isAr ? 'إيقاف المحرك' : 'Engine stop';
      case 'engineResume':
        return I18n.isAr ? 'تشغيل المحرك' : 'Engine resume';
      case 'getLocation':
        return I18n.isAr ? 'الموقع الحالي' : 'Current location';
      case 'sos':
        return 'SOS';
      case 'custom':
        return I18n.isAr ? 'أمر مخصص' : 'Custom command';
      default:
        return t;
    }
  }
  ...
  (String, Color) _statusStyle(String s) {
    switch (s) {
      case 'success':
        return (I18n.isAr ? 'تم بنجاح' : 'Success', const Color(0xFF4CAF50));
      case 'failed':
        return (I18n.isAr ? 'فشل' : 'Failed', const Color(0xFFEF5350));
      case 'queued':
        return (I18n.isAr ? 'في الانتظار' : 'Queued', const Color(0xFFF59E0B));
      default:
        return (I18n.isAr ? 'تم الإرسال' : 'Sent', const Color(0xFF2196F3));
    }
  }
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
              '${I18n.isAr ? 'تاريخ الأوامر' : 'Command history'} - ${widget.device.name}',
              style: const TextStyle(
        ...
        body: _loading
            ...
            : _items.isEmpty
                ? Center(
                    child: Text(
                        I18n.isAr ? 'لا توجد أوامر بعد' : 'No commands yet',
                        style: const TextStyle(
                            color: Color(0xFF8892A4), fontFamily: 'Cairo')))
                : RefreshIndicator(
                    onRefresh: () => _load(),
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        final it = _items[i];
                        final st =
                            _statusStyle((it['status'] ?? '').toString());
...
class _ReplayModal extends StatefulWidget {
  ...
  const _ReplayModal({required this.device}) : onRouteLoaded = null;
...
class _ReplayModalState extends State<_ReplayModal> {
  ...
  Future<BitmapDescriptor> _buildArrowIcon(
      double angleDeg, double speedKmh) async {
    const size = 56.0;
    final recorder = ui.PictureRecorder();
    final canvas = Canvas(recorder);
    ...
    // السهم الرئيسي
    final arrowColor =
        speedKmh > 100 ? const Color(0xFFC41E3A) : const Color(0xFF6BA539);
  ...
  double _calcBearing(double lat1, double lon1, double lat2, double lon2) {
    double toRad(double d) => d * 3.14159265358979 / 180;
  ...
  Set<Polyline> _buildColoredRoute(
      List<Map<String, dynamic>> positions, double speedLimit) {
    ...
    for (int i = 0; i < positions.length - 1; i++) {
      ...
      final color =
          spd > speedLimit ? const Color(0xFFC41E3A) : const Color(0xFF6BA539);
  ...
  Set<Marker> _buildStopMarkers(List<Map<String, dynamic>> positions) {
    ...
    for (int i = 0; i < positions.length; i++) {
      ...
      if (spd < stopSpeed) {
        ...
        if (stopStart != null) {
          final t1 = DateTime.tryParse(
              positions[stopStart]['fixTime']?.toString() ?? '');
          final t2 =
              DateTime.tryParse(positions[i - 1]['fixTime']?.toString() ?? '');
          if (t1 != null && t2 != null) {
                ...
                children: [
                  if (_positions.isEmpty) ...[
                    ...
                    if (_period == 'custom') ...[
                      ...
                      _dateTimeRow(
                          tr('lbl_from2'),
                          _customFrom,
                          (dt) => setState(() => _customFrom = dt),
                          const Color(0xFF6BA539)),
                      const SizedBox(height: 5),
                      _dateTimeRow(
                          tr('lbl_to2'),
                          _customTo,
                          (dt) => setState(() => _customTo = dt),
                          const Color(0xFFC41E3A)),
                    ],
                  ...
                  if (_loaded && _routeData != null) ...[
                    ...
                    Row(children: [
                      _TripStat(
                          value:
                              '${_routeData!['distance'] ?? 0} ${tr('unit_km')}',
                          label: tr('ts_distance')),
                      const SizedBox(width: 5),
                      _TripStat(
                          value:
                              '${_routeData!['max_speed'] ?? 0} ${tr('unit_kmh')}',
                          label: tr('ts_max_speed'),
                          color: const Color(0xFFEF5350)),
                      const SizedBox(width: 5),
                      _TripStat(
                          value: _routeData!['duration'] ?? '-',
                          label: tr('ts_duration'),
                  ...
                  if (_positions.isNotEmpty) ...[
                    ...
                    Row(
                        ...
                        children: [
                          Text(
                            _formatTime(
                                _positions[_currentIdx]['fixTime']?.toString()),
                            style: const TextStyle(
                                color: Color(0xFFA5F3FC),
                                fontSize: 11,
                                fontFamily: 'Cairo'),
                  ...
                  if (_positions.isNotEmpty)
                    Align(
                      ...
                      child: OutlinedButton.icon(
                        ...
                        label: Text('${tr('clear')} ${tr('ctl_route')}',
                            style: const TextStyle(
                                color: Color(0xFFAAAAAA),
                                fontFamily: 'Cairo',
                                fontSize: 11)),
  ...
  Widget _dateTimeRow(
      String label, DateTime? value, Function(DateTime) onPick, Color color) {
    return Row(children: [
      Text(label,
          style: TextStyle(color: color, fontSize: 10, fontFamily: 'Cairo')),
      const SizedBox(width: 6),
      Expanded(
        child: GestureDetector(
          onTap: () async {
    ...
    final date = await showDatePicker(
      ...
      builder: (ctx, child) => Theme(
        data: ThemeData.dark().copyWith(
            colorScheme: const ColorScheme.dark(primary: Color(0xFFC41E3A))),
        child: child!,
      ),
    );
  ...
  Future<void> _loadRoute() async {
    ...
    if (_loaded && result['data'] != null) {
      final filtered = List<Map<String, dynamic>>.from((result['data'] as List)
...
class _ReportsModalState extends State<_ReportsModal> {
  ...
  Map<String, String> _reportSummary = {};
  ...
  late final TextEditingController _spdLimitCtrl = TextEditingController(
      text: (widget.device.speedLimit > 0 ? widget.device.speedLimit : 100)
          .toStringAsFixed(0));

  @override
  void dispose() {
    _spdLimitCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _exportReportPdf() async {
    ...
    try {
      ...
      final titleMap = {
        'trips': tr('rep_trips'),
        'speed': tr('rep_speed'),
        'geofence': tr('rep_geofence'),
        'ignition': tr('rep_ignition'),
      ...
      if (rt == 'trips') {
        ...
        data = _reportRows
            .map((r) => [
                  s(r['time']),
                  (r['status'] == 'stop' ? tr('rs_stop') : tr('rs_drive')),
                  s(r['duration']),
                  s(r['value']),
                  s(r['maxSpeed']),
                  s(r['address']),
      ...
      doc.addPage(pw.MultiPage(
        ...
        header: (ctx) => pw.Container(
          ...
          child: pw.Column(
              ...
              children: [
                pw.Text('$title — ${widget.device.name}',
                    style: pw.TextStyle(
                        font: bold,
                        fontSize: 15,
                        color: const PdfColor.fromInt(0xFFC41E3A))),
                if (_reportSummary.isNotEmpty)
                  pw.Text(_summaryLine(rt),
                      style: pw.TextStyle(
                          font: reg, fontSize: 9, color: PdfColors.grey700)),
        ...
        footer: (ctx) => pw.Align(
            ...
            child: pw.Text(
                '${ctx.pageNumber}/${ctx.pagesCount}  •  himaya-track.com',
                style: pw.TextStyle(
                    font: reg, fontSize: 8, color: PdfColors.grey))),
        build: (ctx) => [
          pw.TableHelper.fromTextArray(
            ...
            headerDecoration:
                const pw.BoxDecoration(color: PdfColor.fromInt(0xFF1E293B)),
            cellStyle: pw.TextStyle(font: reg, fontSize: 8),
            cellAlignment: pw.Alignment.centerRight,
            headerAlignment: pw.Alignment.centerRight,
            border: pw.TableBorder.all(color: PdfColors.grey300, width: 0.5),
            oddRowDecoration:
                const pw.BoxDecoration(color: PdfColor.fromInt(0xFFF5F6F8)),
          ),
        ],
      ));
      await Printing.layoutPdf(
          onLayout: (format) => doc.save(),
          name: '${title}_${widget.device.name}');
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content:
                Text(tr('failed'), style: const TextStyle(fontFamily: 'Cairo')),
            backgroundColor: const Color(0xFFC41E3A)));
    } finally {
  ...
  String _summaryLine(String rt) {
    ...
    if (rt == 'geofence')
      return '${tr('sm_enters')}: ${sm['enters'] ?? '0'}  |  ${tr('sm_exits')}: ${sm['exits'] ?? '0'}';
    final parts = <String>[];
    if (sm['dist'] != null)
      parts.add('${tr('sm_distance')}: ${sm['dist']} ${tr('unit_km')}');
    if (sm['dur'] != null) parts.add('${tr('sm_duration')}: ${sm['dur']}');
    if (sm['maxSpd'] != null)
      parts.add('${tr('sm_max_speed')}: ${sm['maxSpd']} ${tr('unit_kmh')}');
    if (sm['count'] != null)
      parts.add(
          '${rt == 'speed' ? tr('sm_overspeeds') : tr('sm_trips')}: ${sm['count']}');
    return parts.join('  |  ');
  }
                  ...
                  Row(children: [
                    Expanded(
                      ...
                      onTap: () async {
                        ...
                        if (d != null)
                          setState(() =>
                              _fromDate = d.toIso8601String().substring(0, 10));
                      },
                      child: Container(
                          padding: const EdgeInsets.symmetric(
                          ...
                          child: Row(children: [
                            const Icon(Icons.calendar_today,
                                color: Color(0xFF6BA539), size: 14),
                            const SizedBox(width: 6),
                            Expanded(
                                child: Text(tr('lbl_from') + _fromDate,
                                    style: const TextStyle(
                                        color: Colors.white,
                                        fontFamily: 'Cairo',
                                        fontSize: 11)))
                    ...
                    Expanded(
                      ...
                      onTap: () async {
                        ...
                        if (d != null)
                          setState(() =>
                              _toDate = d.toIso8601String().substring(0, 10));
                      },
                      child: Container(
                          padding: const EdgeInsets.symmetric(
                              horizontal: 10, vertical: 10),
                          ...
                          child: Row(children: [
                            const Icon(Icons.calendar_today,
                                color: Color(0xFFC41E3A), size: 14),
                            const SizedBox(width: 6),
                            Expanded(
                                child: Text(tr('lbl_to') + _toDate,
                                    style: const TextStyle(
                                        color: Colors.white,
                                        fontFamily: 'Cairo',
                                        fontSize: 11)))
                                ...
                                Expanded(
                                    child: Column(
                                        crossAxisAlignment:
                                            CrossAxisAlignment.start,
                                        children: [
                                      Text(tr(rt['label'] as String),
                                          style: const TextStyle(
                                              color: Color(0xFFE8EAF0),
                                              fontSize: 11,
                                              fontWeight: FontWeight.w500,
                                              fontFamily: 'Cairo')),
                                      Text(tr(rt['sub'] as String),
                                          style: const TextStyle(
                                              color: Color(0x60FFFFFF),
                                              fontSize: 9,
                                              fontFamily: 'Cairo')),
                              ...
                              child: Column(
                                children: [
                                  if (_openReport != 'trips' &&
                                      _openReport != 'speed' &&
                                      _openReport != 'geofence')
                                    Container(
                                      padding: const EdgeInsets.symmetric(
                                          horizontal: 8, vertical: 4),
                                      color: const Color(0xFF1E293B),
                                      child: Row(children: [
                                        _TableHead(_openReport == 'trips'
                                            ? tr('th_start_time')
                                            : tr('th_time')),
                                        _TableHead(_openReport == 'trips'
                                            ? tr('th_distance')
                                            : tr('th_value')),
                                        _TableHead(tr('th_location')),
                                      ]),
                                    ),
                                  if (_openReport == 'geofence' &&
                                      _reportSummary.isNotEmpty)
                                    Container(
                                      ...
                                      child: Row(children: [
                                        _TripStat(
                                            value:
                                                _reportSummary['enters'] ?? '0',
                                            label: tr('sm_enters'),
                                            color: const Color(0xFF27AE60)),
                                        const SizedBox(width: 4),
                                        _TripStat(
                                            value:
                                                _reportSummary['exits'] ?? '0',
                                            label: tr('sm_exits'),
                                            color: const Color(0xFFEF5350)),
                                      ]),
                                    ),
                                  if ((_openReport == 'trips' ||
                                    ...
                                    Container(
                                      ...
                                      child: Row(children: [
                                        _TripStat(
                                            value:
                                                '${_reportSummary['dist']} ${tr('unit_km')}',
                                            label: tr('sm_distance'),
                                            color: const Color(0xFF6BA539)),
                                        const SizedBox(width: 4),
                                        _TripStat(
                                            value:
                                                _reportSummary['dur'] ?? '--',
                                            label: tr('sm_duration'),
                                            color: const Color(0xFF38bdf8)),
                                        const SizedBox(width: 4),
                                        _TripStat(
                                            value:
                                                '${_reportSummary['maxSpd']} ${tr('unit_kmh')}',
                                            label: tr('sm_max_speed'),
                                            color: const Color(0xFFC41E3A)),
                                        const SizedBox(width: 4),
                                        _TripStat(
                                            value:
                                                _reportSummary['count'] ?? '0',
                                            label: _openReport == 'speed'
                                                ? tr('sm_overspeeds')
                                                : tr('sm_trips')),
                                      ]),
                                    ),
                                  if (_openReport != 'trips' &&
                                    ...
                                    Container(
                                      ...
                                      color: const Color(0xFF1E293B),
                                      child: Row(children: [
                                        _TableHead(tr('th_time')),
                                        _TableHead(tr('th_value')),
                                        _TableHead(tr('th_location')),
                                      ]),
                                    ),
                                  if (_openReport == 'trips')
                                                          ...
                                                          decoration: BoxDecoration(
                                                              ...
                                                              borderRadius:
                                                                  BorderRadius.circular(
                                                                      6)),
                                                          child: Text(r['status'] == 'stop' ? tr('rs_stop') : tr('rs_drive'),
                                                              style: TextStyle(
                                                                  color: r['status'] ==
                                                                          'stop'
  ...
  Future<void> _fetchAddresses() async {
    ...
    for (int i = 0; i < _reportRows.length; i++) {
      ...
      final isCoord = addrStr.contains(',') &&
          double.tryParse(addrStr.split(',')[0].trim()) != null;
      if (isCoord) {
        final a = await resolve(addrStr);
        if (mounted)
          setState(() {
            _reportRows[i] = {...r, 'address': a};
          });
        await Future.delayed(const Duration(milliseconds: 400));
      }
      if (!mounted) break;
      final endStr = ((_reportRows[i] as Map)['endAddress'] ?? '').toString();
      final isEndCoord = endStr.contains(',') &&
          double.tryParse(endStr.split(',')[0].trim()) != null;
      if (isEndCoord) {
        final a2 = await resolve(endStr);
        if (mounted)
          setState(() {
            _reportRows[i] = {
              ...(_reportRows[i] as Map<String, dynamic>),
              'endAddress': a2
            };
          });
        await Future.delayed(const Duration(milliseconds: 400));
  ...
  Future<String> _getAddress(double lat, double lng) async {
    ...
    if (_AsyncAddressText._cache.containsKey(k))
      return _AsyncAddressText._cache[k]!;
    try {
      final r = await ApiService.request(
          'reverse_geocode', {'lat': lat, 'lng': lng, 'lang': I18n.lang});
  ...
  Future<void> _toggleReport(String reportId) async {
    ...
    if (fromDt != null && toDt != null) {
      ...
      if (diff < 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(tr('rep_end_before_start'),
                style: const TextStyle(fontFamily: 'Cairo')),
            backgroundColor: const Color(0xFFC41E3A)));
        return;
      }
      if (diff > 3) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(tr('rep_max_3_days'),
                style: const TextStyle(fontFamily: 'Cairo')),
            backgroundColor: const Color(0xFFC41E3A)));
        return;
      }
    ...
    try {
      ...
      switch (reportId) {
        case 'speed':
          ...
          final allPos =
              List<dynamic>.from(spResult['data'] ?? spResult['events'] ?? []);
          final speedLimit = double.tryParse(_spdLimitCtrl.text.trim()) ??
              (widget.device.speedLimit > 0 ? widget.device.speedLimit : 100);
          // group consecutive overspeed positions
          final spGroups = <Map<String, dynamic>>[];
          Map<String, dynamic>? spGrp;
          for (final p in allPos) {
            final spd =
                (((p['speed'] ?? 0) as num).toDouble() * 1.852).roundToDouble();
          ...
          final mappedSpeed = spGroups.map((g) {
            ...
            for (int i = 1; i < positions.length; i++) {
              final lat1 =
                  ((positions[i - 1]['latitude'] ?? 0) as num).toDouble() *
                      3.14159265 /
                      180;
              final lon1 =
                  ((positions[i - 1]['longitude'] ?? 0) as num).toDouble() *
                      3.14159265 /
                      180;
              final lat2 = ((positions[i]['latitude'] ?? 0) as num).toDouble() *
                  3.14159265 /
                  180;
              final lon2 =
                  ((positions[i]['longitude'] ?? 0) as num).toDouble() *
                      3.14159265 /
                      180;
            ...
            final st = (startPos['fixTime'] ?? startPos['serverTime'] ?? '')
                .toString();
            final et =
                (endPos['fixTime'] ?? endPos['serverTime'] ?? '').toString();
            int durSec = 0;
            try {
              durSec = DateTime.parse(et)
            ...
            spTotalDist += dist;
            spTotalSec += durSec;
            if ((g['maxSpd'] as double) > spMaxSpd)
              spMaxSpd = g['maxSpd'] as double;
            ...
            return <String, dynamic>{
              ...
              'value':
                  '${(g['maxSpd'] as double).toStringAsFixed(0)} ${tr('unit_kmh')}',
              'minSpeed':
                  '${(g['minSpd'] as double).toStringAsFixed(0)} ${tr('unit_kmh')}',
              'maxSpeed':
                  '${(g['maxSpd'] as double).toStringAsFixed(0)} ${tr('unit_kmh')}',
              'distance': '${dist.toStringAsFixed(3)} ${tr('unit_km')}',
              'points': positions.length.toString(),
              'address': sLat != 0
                  ? '${sLat.toStringAsFixed(5)}, ${sLon.toStringAsFixed(5)}'
                  : '--',
              'endAddress': eLat != 0
                  ? '${eLat.toStringAsFixed(5)}, ${eLon.toStringAsFixed(5)}'
                  : '--',
              'status': 'speed',
            };
          }).toList();
          final spHrs = spTotalSec ~/ 3600;
          final spMins = (spTotalSec % 3600) ~/ 60;
          _reportSummary = {
            'count': '${spGroups.length}',
            'dist': spTotalDist.toStringAsFixed(1),
            'maxSpd': spMaxSpd.toStringAsFixed(0),
            'dur': spHrs > 0
                ? tr('dur_hm', {'h': '$spHrs', 'm': '$spMins'})
                : tr('dur_min', {'m': '$spMins'}),
          };
          setState(() {
            _loading = false;
            _reportRows = List<dynamic>.from(mappedSpeed);
          });
          _fetchAddresses();
          return;
        ...
        case 'ignition':
          ...
          final ignRows = List<dynamic>.from(result['data'] ??
              result['events'] ??
        ...
        case 'operation':
          ...
          final trips = List<dynamic>.from(
              result['data'] ?? (result is List ? result : []));
          setState(() {
            _loading = false;
            _reportRows = trips;
          });
        ...
        case 'geofence':
          ...
          final geoEvents =
              List<dynamic>.from(result['data'] ?? result['events'] ?? []);
          String fmtGeoTime(String s) {
            try {
              final dt = DateTime.parse(s).toLocal();
              return '${dt.year}-${dt.month.toString().padLeft(2, '0')}-${dt.day.toString().padLeft(2, '0')} ${dt.hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')}';
            } catch (_) {
          ...
          geoEvents.sort((a, b) => (b['eventTime'] ?? '')
              .toString()
              .compareTo((a['eventTime'] ?? '').toString()));
          int gfEnters = 0;
          int gfExits = 0;
          final mappedGeo = geoEvents.map((e) {
            ...
            return <String, dynamic>{
              ...
              'geofenceName':
                  (e['geofenceName'] ?? e['geofenceId'] ?? tr('rt_geofence'))
                      .toString(),
          ...
          setState(() {
            _loading = false;
            _reportRows = List<dynamic>.from(mappedGeo);
          });
          _fetchAddresses();
          return;
        ...
        case 'trips':
          final traccarId =
              widget.device.traccarId > 0 ? widget.device.traccarId : devId;
          result = await ApiService.request('get_trips_mysql', {
            'deviceId': traccarId,
            'from': from,
            'to': to,
          });
          final tripsList = List<dynamic>.from(
              result['data'] ?? (result is List ? result : []));
          double totalDist = 0;
          double maxSpd = 0;
          int totalMs = 0;
          ...
          final durStr = hrs > 0
              ? tr('dur_hm', {'h': '$hrs', 'm': '$mins'})
              : tr('dur_min', {'m': '$mins'});
          _reportSummary = {
            'dist': totalKm,
            'dur': durStr,
            'maxSpd': maxSpd.toStringAsFixed(0),
          ...
          final mappedTrips = tripsList.map((r) {
            ...
            final dist = isStop
                ? '0.00 ${tr('unit_km')}'
                : '${((r['distance'] ?? 0) as num).toStringAsFixed(2)} ${tr('unit_km')}';
            String calcDur() {
              try {
                final s = DateTime.parse(st);
                final e = DateTime.parse(et);
                final diff = e.difference(s).abs();
                final h = diff.inHours;
                final m = diff.inMinutes % 60;
                final s2 = diff.inSeconds % 60;
            ...
            final maxSpd = isStop
                ? '0 ${tr('unit_kmh')}'
                : '${((r['maxSpeed'] ?? 0) as num).toStringAsFixed(0)} ${tr('unit_kmh')}';
            ...
            return {
              ...
              'maxSpeed': maxSpd,
              'address': startLat != 0
                  ? '${startLat.toStringAsFixed(5)}, ${startLon.toStringAsFixed(5)}'
                  : '--',
              'endAddress': isStop
                  ? '—'
                  : (endLat != 0
                      ? '${endLat.toStringAsFixed(5)}, ${endLon.toStringAsFixed(5)}'
                      : '--'),
          ...
          setState(() {
            _loading = false;
            _reportRows = List<dynamic>.from(mappedTrips);
          });
          _fetchAddresses();
          return;
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(children: [
          ...
          Text(label,
              style: const TextStyle(
                  color: Color(0x60FFFFFF), fontSize: 8, fontFamily: 'Cairo')),
        ]),
      ),
    );
  }
...
class _InfoTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label,
            style: const TextStyle(
                color: Color(0x60FFFFFF), fontSize: 9, fontFamily: 'Cairo')),
        Text(value,
            style: TextStyle(
                color: valueColor,
                fontSize: 11,
...
class _TableHead extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(
      child: Text(text,
          style: const TextStyle(
              color: Color(0x70FFFFFF), fontSize: 9, fontFamily: 'Cairo')));
}
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: hint,
        hintStyle: const TextStyle(
            color: Color(0x50FFFFFF), fontFamily: 'Cairo', fontSize: 11),
        filled: true,
        ...
        focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(8),
            borderSide: const BorderSide(color: Color(0xFFC41E3A))),
...
class _RoutePlaybackScreenState extends State<_RoutePlaybackScreen> {
  ...
  Future<void> _preloadImages() async {
    for (final e in const {'norm': 'blue', 'over': 'red'}.entries) {
      ...
      try {
        ...
        _iconCache[key] = BitmapDescriptor.bytes(data.buffer.asUint8List(),
            imagePixelRatio: 6.0);
      } catch (_) {}
    }
  ...
  Future<BitmapDescriptor> _renderCarIcon(bool isOver, double angleDeg) async {
    ...
    final rec = ui.PictureRecorder();
    final c = Canvas(rec, const Rect.fromLTWH(0, 0, SZ * scale, SZ * scale));
    c.scale(scale);
    ...
    final bodyColor =
        isOver ? const Color(0xFFB03A2E) : const Color(0xFF1A5276);

    // ?? Drop shadow ????????????????????????????????????????????????
    c.drawRRect(
      RRect.fromRectAndRadius(const Rect.fromLTWH(bL + 3, bT + 4, bW, bH),
    ...
    c.drawPath(
        ...
        Paint()
          ..color =
              bodyColor.withRed((bodyColor.red * 1.15).clamp(0, 255).toInt()));
    ...
    c.drawPath(
        ...
        Paint()
          ..color =
              bodyColor.withRed((bodyColor.red * 0.85).clamp(0, 255).toInt()));

    // ?? Main roof body ?????????????????????????????????????????????
    c.drawRRect(
      RRect.fromRectXY(const Rect.fromLTWH(bL, bT, bW, bH), 6, 6),
      Paint()
        ..isAntiAlias = true
        ..shader = ui.Gradient.linear(
          const Offset(bL, 0),
          const Offset(bL + bW, 0),
          [
    ...
    c.drawRRect(
      RRect.fromRectXY(
          const Rect.fromLTWH(bL + 3, bT + bH * 0.28, bW - 6, bH * 0.44), 4, 4),
      Paint()..color = bodyColor.withValues(alpha: 0.7),
    );
    ...
    final wfPath = Path()
      ...
      ..close();
    c.drawPath(wfPath, Paint()..color = const Color(0xCC90CAF9));
    ...
    final glarePath = Path()
      ...
      ..close();
    c.drawPath(
        glarePath, Paint()..color = Colors.white.withValues(alpha: 0.35));
    ...
    final wrPath = Path()
      ...
      ..close();
    c.drawPath(wrPath, Paint()..color = const Color(0x9990CAF9));

    // ?? Side mirrors (tiny - realistic) ???????????????????????????
    // Left mirror
    c.drawRRect(
      RRect.fromRectXY(const Rect.fromLTWH(bL - 4, bT + bH * 0.2, 5, 4), 1, 1),
      Paint()..color = bodyColor.withValues(alpha: 0.9),
    );
    // Right mirror
    c.drawRRect(
      RRect.fromRectXY(
          const Rect.fromLTWH(bL + bW - 1, bT + bH * 0.2, 5, 4), 1, 1),
      Paint()..color = bodyColor.withValues(alpha: 0.9),
    );

    // ?? Hood crease line ???????????????????????????????????????????
    c.drawLine(
      const Offset(CX, bT + bH * 0.0),
      const Offset(CX, bT + bH * 0.12),
      Paint()
    ...
    c.drawRRect(
      RRect.fromRectXY(const Rect.fromLTWH(bL + 1, bT + 1, 7, 3), 1, 1),
      Paint()..color = const Color(0xFFFFEE88),
    );
    c.drawRRect(
      RRect.fromRectXY(const Rect.fromLTWH(bL + bW - 8, bT + 1, 7, 3), 1, 1),
      Paint()..color = const Color(0xFFFFEE88),
    );

    // ?? Tail lights (red) ??????????????????????????????????????????
    c.drawRRect(
      RRect.fromRectXY(const Rect.fromLTWH(bL + 1, bT + bH - 4, 7, 3), 1, 1),
      Paint()..color = const Color(0xFFEF5350),
    );
    c.drawRRect(
      RRect.fromRectXY(
          const Rect.fromLTWH(bL + bW - 8, bT + bH - 4, 7, 3), 1, 1),
      Paint()..color = const Color(0xFFEF5350),
    );
  ...
  Future<void> _loadRoute() async {
    ...
    if (!hasMovement && raw.length >= 2) {
      ...
      final dLat = ((double.tryParse('${l['latitude']}') ?? 0) -
              (double.tryParse('${f['latitude']}') ?? 0))
          .abs();
      final dLng = ((double.tryParse('${l['longitude']}') ?? 0) -
              (double.tryParse('${f['longitude']}') ?? 0))
          .abs();
  ...
  Future<void> _buildDirArrowIcon() async {
    ...
    final path = Path()
      ...
      ..lineTo(3, sz - 3)
      ..close();
    c.drawPath(path, Paint()..color = Colors.white.withValues(alpha: 0.95));
    c.drawPath(
        path,
        Paint()
    ...
    final data = await img.toByteData(format: ui.ImageByteFormat.png);
    if (data != null)
      _dirArrowIcon = BitmapDescriptor.fromBytes(data.buffer.asUint8List());
  }
  ...
  void _buildPolylines() {
    ...
    for (final p in _positions) {
      ...
      final newCol =
          spd > lim ? const Color(0xFFC41E3A) : const Color(0xFF6BA539);
  ...
  Future<void> _buildPIcon() async {
    ...
    final data = await img.toByteData(format: ui.ImageByteFormat.png);
    if (data != null)
      _pIcon = BitmapDescriptor.fromBytes(data.buffer.asUint8List());
    _pIcon ??=
  ...
  Future<BitmapDescriptor> _numStopIcon(int n) async {
    ...
    const cx = w / 2, cy = r + 2;
    final rec = ui.PictureRecorder();
    final c = Canvas(rec, const Rect.fromLTWH(0, 0, w * scale, h * scale));
    c.scale(scale);
  ...
  Future<void> _buildStopMarkers() async {
    ...
    for (int i = 0; i < _positions.length; i++) {
      ...
      if (spd < stopSpd) {
        ...
        final t1 =
            DateTime.tryParse(_positions[start]['fixTime']?.toString() ?? '');
        final t2 =
            DateTime.tryParse(_positions[i - 1]['fixTime']?.toString() ?? '');
        if (t1 != null &&
            t2 != null &&
            t2.difference(t1).inSeconds >= stopSec) {
          ...
          if (lat != 0) {
            ...
            _stopMarkersSet.add(Marker(
              ...
              onTap: () => setState(() => _selectedStop = {
                    ...
                    'date':
                        '${t1.toLocal().year}-${t1.toLocal().month.toString().padLeft(2, '0')}'
  ...
  void _fitBounds() {
    ...
    final lats = _positions
        .map((p) => double.tryParse(p['latitude'].toString()) ?? 0)
        .where((v) => v != 0);
    final lngs = _positions
        .map((p) => double.tryParse(p['longitude'].toString()) ?? 0)
        .where((v) => v != 0);
  ...
  Future<void> _maybeRecenter(double lat, double lng) async {
    ...
    try {
      ...
      final inside = lat >= sw.latitude + latM &&
          lat <= ne.latitude - latM &&
          lng >= sw.longitude + lngM &&
          lng <= ne.longitude - lngM;
  ...
  void _seekTo(int origIdx) {
    ...
    for (int i = 0; i < _interp.length; i++) {
      if ((_interp[i]['origIdx'] as int) >= _origIdx) {
        _interpIdx = i;
        break;
      }
    }
    ...
    if (lat != 0)
      _ctrl?.animateCamera(CameraUpdate.newLatLng(LatLng(lat, lng)));
  }

  void _skipForward() {
    final next = _stopIndices.where((i) => i > _origIdx).toList();
    _seekTo(next.isNotEmpty
        ? next.first
        : (_origIdx + _positions.length ~/ 10).clamp(0, _positions.length - 1));
  }

  void _skipBackward() {
    final prev = _stopIndices.where((i) => i < _origIdx - 2).toList();
    _seekTo(prev.isNotEmpty
        ? prev.last
        : (_origIdx - _positions.length ~/ 10).clamp(0, _positions.length - 1));
  }
  ...
  void _showSummary() {
    ...
    double? odoOf(Map p) {
      ...
      if (a is Map && a['totalDistance'] is num)
        return (a['totalDistance'] as num).toDouble();
      return null;
    }
    ...
    final maxSpd = _positions
        ...
        .reduce(math.max)
        .toStringAsFixed(1);
    final dateStr = t1 != null ? '${t1.day}/${t1.month}/${t1.year}' : '-';
      ...
      builder: (dialogCtx) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Row(children: [
              _SummaryCard(tr('ts_distance'), distStr, tr('unit_km'),
                  const Color(0xFF6BA539)),
              const SizedBox(width: 8),
              _SummaryCard(
                  tr('ts_duration'), durStr, '', const Color(0xFF38bdf8)),
            ]),
            const SizedBox(height: 8),
            Row(children: [
              _SummaryCard(tr('ts_max_speed'), maxSpd, tr('unit_kmh'),
                  const Color(0xFFC41E3A)),
              const SizedBox(width: 8),
              _SummaryCard(tr('ts_points'), '${_positions.length}', '',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(children: [
              ...
              child: Material(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(14),
                    border: Border.all(
                        color: const Color(0xFFFF6F00).withValues(alpha: .6)),
                    boxShadow: const [
                      BoxShadow(
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Row(children: [
                      ...
                      Text(
                          '${_selectedStop!['from']} - ${_selectedStop!['to']}',
                          style: const TextStyle(
                              fontFamily: 'Cairo',
                              color: Colors.white70,
                              fontSize: 12),
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                SliderTheme(
                  data: const SliderThemeData(
                    ...
                    activeTrackColor: Color(0xFF6BA539),
                    inactiveTrackColor: Colors.white24,
        ...
        if (_loading) const _CinematicLoader(),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) => Expanded(
        child: Container(
          ...
          child: Column(children: [
            Text(label,
                style: const TextStyle(
                    color: Colors.white54, fontFamily: 'Cairo', fontSize: 11)),
            const SizedBox(height: 4),
            Text(value,
                style: TextStyle(
                    color: valueColor,
...
class _VehicleIconPainter extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    ...
    final roof = Paint()
      ..color = Color.fromARGB(255, (color.red * 0.75).round(),
          (color.green * 0.75).round(), (color.blue * 0.75).round());
    final glass = Paint()..color = const Color(0xCC90CAF9);
    final glare = Paint()..color = Colors.white.withValues(alpha: 0.22);
    final bdr = Paint()
      ..color = Colors.black.withValues(alpha: 0.45)
    ...
    final mir = Paint()..color = color.withValues(alpha: 0.85);
    final x = r.left, y = r.top, w = r.width, h = r.height;

    switch (type) {
      case 'car':
        _s(c, x, y, w, h, color, body, roof, glass, glare, bdr, hl, tl, mir);
        break;
      case 'suv':
        _suv(c, x, y, w, h, color, body, roof, glass, glare, bdr, hl, tl, mir);
        break;
      case 'pickup':
        _pu(c, x, y, w, h, color, body, roof, glass, glare, bdr, hl, tl, mir);
        break;
      case 'van':
        _van(c, x, y, w, h, color, body, roof, glass, glare, bdr, hl, tl);
        break;
      case 'bus':
        _bus(c, x, y, w, h, color, body, roof, glass, bdr, hl, tl);
        break;
      case 'truck':
        _trk(c, x, y, w, h, color, body, roof, glass, bdr, hl, tl);
        break;
      case 'motorcycle':
        _moto(c, x, y, w, h, color, body, bdr);
        break;
      case 'tuk_tuk':
        _tuk(c, x, y, w, h, color, body, roof, glass, bdr, hl, tl);
        break;
      case 'excavator':
        _exc(c, x, y, w, h, body, roof, bdr);
        break;
      case 'tractor':
        _trac(c, x, y, w, h, color, body, roof, bdr);
        break;
      case 'boat':
        _boat(c, x, y, w, h, color, body, glass, bdr);
        break;
      case 'person':
        c.drawCircle(Offset(x + w / 2, y + h * 0.35), w * 0.3, body);
        c.drawOval(Rect.fromLTWH(x + w * 0.2, y + h * 0.6, w * 0.6, h * 0.33),
  ...
  void _exc(Canvas c, double x, double y, double w, double h, Paint body,
      Paint roof, Paint bdr) {
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .05, y + h * .40, w * .17, h * .56), 3, 3),
        roof);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .78, y + h * .40, w * .17, h * .56), 3, 3),
        roof);
    final hull = RRect.fromRectXY(
        Rect.fromLTWH(x + w * .23, y + h * .46, w * .54, h * .48), 5, 5);
    c.drawRRect(hull, body);
    c.drawRRect(hull, bdr);
    final arm = RRect.fromRectXY(
        Rect.fromLTWH(x + w * .43, y + h * .15, w * .14, h * .34), 3, 3);
    c.drawRRect(arm, body);
    c.drawRRect(arm, bdr);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .33, y + h * .02, w * .34, h * .15), 3, 3),
        roof);
  }
  ...
  void _s(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .06, y + h * .05, w * .88, h * .9), 10, 10),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .14, y + h * .28, w * .72, h * .42), 6, 6),
        roof);
    c.drawPath(
        Path()
          ..moveTo(x + w * .18, y + h * .28)
          ..lineTo(x + w * .82, y + h * .28)
          ..lineTo(x + w * .76, y + h * .12)
          ..lineTo(x + w * .24, y + h * .12)
          ..close(),
        glass);
    c.drawPath(
        Path()
          ..moveTo(x + w * .22, y + h * .26)
          ..lineTo(x + w * .4, y + h * .26)
          ..lineTo(x + w * .36, y + h * .13)
          ..lineTo(x + w * .22, y + h * .13)
          ..close(),
        glare);
    c.drawPath(
        Path()
          ..moveTo(x + w * .2, y + h * .7)
          ..lineTo(x + w * .8, y + h * .7)
          ..lineTo(x + w * .76, y + h * .86)
          ..lineTo(x + w * .24, y + h * .86)
          ..close(),
        Paint()..color = const Color(0x9990CAF9));
    c.drawRRect(
        RRect.fromRectXY(Rect.fromLTWH(x, y + h * .22, w * .07, h * .06), 1, 1),
        mir);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .93, y + h * .22, w * .07, h * .06), 1, 1),
        mir);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .05, w * .28, h * .05), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .62, y + h * .05, w * .28, h * .05), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .9, w * .28, h * .05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .62, y + h * .9, w * .28, h * .05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .06, y + h * .05, w * .88, h * .9), 10, 10),
  ...
  void _suv(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .04, y + h * .04, w * .92, h * .92), 9, 9),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .26, w * .8, h * .48), 6, 6),
        roof);
    c.drawPath(
        Path()
          ..moveTo(x + w * .14, y + h * .26)
          ..lineTo(x + w * .86, y + h * .26)
          ..lineTo(x + w * .82, y + h * .1)
          ..lineTo(x + w * .18, y + h * .1)
          ..close(),
        glass);
    c.drawPath(
        Path()
          ..moveTo(x + w * .2, y + h * .24)
          ..lineTo(x + w * .38, y + h * .24)
          ..lineTo(x + w * .35, y + h * .11)
          ..lineTo(x + w * .2, y + h * .11)
          ..close(),
        glare);
    c.drawPath(
        Path()
          ..moveTo(x + w * .14, y + h * .74)
          ..lineTo(x + w * .86, y + h * .74)
          ..lineTo(x + w * .82, y + h * .9)
          ..lineTo(x + w * .18, y + h * .9)
          ..close(),
        Paint()..color = const Color(0x9990CAF9));
    c.drawRRect(
        RRect.fromRectXY(Rect.fromLTWH(x, y + h * .2, w * .05, h * .08), 1, 1),
        mir);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .95, y + h * .2, w * .05, h * .08), 1, 1),
        mir);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .08, y + h * .04, w * .3, h * .05), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .62, y + h * .04, w * .3, h * .05), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .08, y + h * .91, w * .3, h * .05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .62, y + h * .91, w * .3, h * .05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .04, y + h * .04, w * .92, h * .92), 9, 9),
  ...
  void _pu(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .06, y + h * .05, w * .88, h * .9), 8, 8),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .12, y + h * .26, w * .76, h * .28), 5, 5),
        roof);
    c.drawPath(
        Path()
          ..moveTo(x + w * .16, y + h * .26)
          ..lineTo(x + w * .84, y + h * .26)
          ..lineTo(x + w * .8, y + h * .12)
          ..lineTo(x + w * .2, y + h * .12)
          ..close(),
        glass);
    c.drawLine(
        Offset(x + w * .08, y + h * .56),
        Offset(x + w * .92, y + h * .56),
        Paint()
          ..color = Colors.black.withValues(alpha: .25)
          ..strokeWidth = 1.5);
    c.drawRRect(
        RRect.fromRectXY(Rect.fromLTWH(x, y + h * .2, w * .07, h * .06), 1, 1),
        mir);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .93, y + h * .2, w * .07, h * .06), 1, 1),
        mir);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .05, w * .28, h * .05), 1, 1),
  ...
  void _van(
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .04, y + h * .04, w * .92, h * .92), 8, 8),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .08, y + h * .25, w * .84, h * .5), 5, 5),
        roof);
    c.drawPath(
        Path()
          ..moveTo(x + w * .1, y + h * .25)
    ...
    c.drawPath(
        ...
        Paint()..color = const Color(0x7790CAF9));
    c.drawLine(
        Offset(x + w * .06, y + h * .42),
        Offset(x + w * .94, y + h * .42),
        Paint()
  ...
  void _bus(Canvas c, double x, double y, double w, double h, Color col,
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .04, y + h * .02, w * .92, h * .96), 7, 7),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .2, w * .8, h * .6), 4, 4),
        roof);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .05, w * .8, h * .13), 3, 3),
        glass);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .82, w * .8, h * .13), 3, 3),
        Paint()..color = const Color(0x7790CAF9));
    for (int i = 0; i < 3; i++) {
      c.drawRRect(
          RRect.fromRectXY(
              Rect.fromLTWH(
                  x + w * .12 + i * w * .28, y + h * .22, w * .22, h * .14),
              2,
              2),
          glass);
    }
    for (int i = 0; i < 3; i++) {
      c.drawRRect(
          RRect.fromRectXY(
              Rect.fromLTWH(
                  x + w * .12 + i * w * .28, y + h * .64, w * .22, h * .14),
              2,
              2),
          glass);
    }
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .08, y + h * .02, w * .3, h * .04), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .62, y + h * .02, w * .3, h * .04), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .08, y + h * .94, w * .3, h * .04), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .62, y + h * .94, w * .3, h * .04), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .04, y + h * .02, w * .92, h * .96), 7, 7),
  ...
  void _trk(Canvas c, double x, double y, double w, double h, Color col,
    ...
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .06, y + h * .38, w * .88, h * .58), 5, 5),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .06, y + h * .04, w * .88, h * .36), 8, 8),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .12, y + h * .2, w * .76, h * .18), 4, 4),
        roof);
    c.drawPath(
        Path()
          ..moveTo(x + w * .14, y + h * .2)
          ..lineTo(x + w * .86, y + h * .2)
          ..lineTo(x + w * .82, y + h * .07)
          ..lineTo(x + w * .18, y + h * .07)
          ..close(),
        glass);
    c.drawLine(
        Offset(x + w * .06, y + h * .55),
        Offset(x + w * .94, y + h * .55),
        Paint()
          ..color = Colors.black.withValues(alpha: .12)
          ..strokeWidth = 1);
    c.drawLine(
        Offset(x + w * .06, y + h * .72),
        Offset(x + w * .94, y + h * .72),
        Paint()
          ..color = Colors.black.withValues(alpha: .12)
          ..strokeWidth = 1);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .04, w * .28, h * .04), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .62, y + h * .04, w * .28, h * .04), 1, 1),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .92, w * .28, h * .04), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .62, y + h * .92, w * .28, h * .04), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .06, y + h * .04, w * .88, h * .92), 5, 5),
        bdr);
  }

  void _moto(Canvas c, double x, double y, double w, double h, Color col,
      Paint body, Paint bdr) {
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .3, y + h * .06, w * .4, h * .7), 8, 8),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .1, y + h * .08, w * .8, h * .08), 3, 3),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .32, y + h * .42, w * .36, h * .28), 5, 5),
        Paint()..color = const Color(0xFF1A1A2E));
    c.drawOval(Rect.fromLTWH(x + w * .38, y + h * .06, w * .24, h * .08),
        Paint()..color = const Color(0xFFFFF9C4));
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .3, y + h * .06, w * .4, h * .7), 8, 8),
  ...
  void _tuk(Canvas c, double x, double y, double w, double h, Color col,
    ...
    final bd = Path()
      ...
      ..lineTo(x + w * .9, y + h * .78)
      ..quadraticBezierTo(x + w * .9, y + h * .94, x + w * .72, y + h * .94)
      ..lineTo(x + w * .28, y + h * .94)
      ..quadraticBezierTo(x + w * .1, y + h * .94, x + w * .1, y + h * .78)
      ..lineTo(x + w * .1, y + h * .2)
      ..quadraticBezierTo(x + w * .28, y + h * .04, x + w * .5, y + h * .04)
      ..close();
    c.drawPath(bd, body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .18, y + h * .24, w * .64, h * .44), 6, 6),
        roof);
    c.drawPath(
        Path()
          ..moveTo(x + w * .22, y + h * .24)
          ..lineTo(x + w * .78, y + h * .24)
          ..lineTo(x + w * .68, y + h * .1)
          ..lineTo(x + w * .32, y + h * .1)
          ..close(),
        glass);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .38, y + h * .04, w * .24, h * .06), 2, 2),
        hl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .2, y + h * .88, w * .22, h * .05), 1, 1),
        tl);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .58, y + h * .88, w * .22, h * .05), 1, 1),
        tl);
    c.drawPath(bd, bdr);
  }

  void _trac(Canvas c, double x, double y, double w, double h, Color col,
      Paint body, Paint roof, Paint bdr) {
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .28, y + h * .04, w * .44, h * .32), 5, 5),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .2, y + h * .3, w * .6, h * .28), 6, 6),
        body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .26, y + h * .34, w * .48, h * .18), 4, 4),
        roof);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .22, y + h * .55, w * .56, h * .36), 6, 6),
        body);
    c.drawLine(
        Offset(x + w * .42, y + h * .04),
        Offset(x + w * .42, y),
        Paint()
          ..color = col
          ..strokeWidth = w * .06
  ...
  void _boat(Canvas c, double x, double y, double w, double h, Color col,
      Paint body, Paint glass, Paint bdr) {
    final hull = Path()
      ..moveTo(x + w * .5, y + h * .04)
      ...
      ..quadraticBezierTo(x + w * .04, y + h * .04, x + w * .5, y + h * .04)
      ..close();
    c.drawPath(hull, body);
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(x + w * .2, y + h * .12, w * .6, h * .2), 5, 5),
        glass);
    c.drawLine(
        Offset(x + w * .1, y + h * .38),
        Offset(x + w * .9, y + h * .38),
...
class _CinematicLoaderState extends State<_CinematicLoader>
  ...
  void initState() {
    ...
    _ctrl = AnimationController(
      ...
      ..repeat();
    _carX = Tween<double>(begin: -0.6, end: 0.6)
  ...
  void paint(Canvas c, Size s) {
    ...
    c.drawPath(
        Path()
          ...
          ..close(),
        Paint()..color = glass.withValues(alpha: 0.6));
    // Headlights
    c.drawRRect(RRect.fromRectXY(const Rect.fromLTWH(4, 3, 9, 3), 1, 1),
        Paint()..color = const Color(0xFFFFF9C4));
    ...
    c.drawRRect(RRect.fromRectXY(Rect.fromLTWH(4, s.height - 6, 9, 3), 1, 1),
        Paint()..color = const Color(0xFFEF5350));
    c.drawRRect(
        RRect.fromRectXY(Rect.fromLTWH(s.width - 13, s.height - 6, 9, 3), 1, 1),
        Paint()..color = const Color(0xFFEF5350));
    // Mirrors
    c.drawRRect(
        RRect.fromRectXY(Rect.fromLTWH(-2, s.height * 0.22, 5, 3), 1, 1),
        Paint()..color = body.withValues(alpha: 0.8));
    c.drawRRect(
        RRect.fromRectXY(
            Rect.fromLTWH(s.width - 3, s.height * 0.22, 5, 3), 1, 1),
        Paint()..color = body.withValues(alpha: 0.8));
    // Outline
    c.drawRRect(
      RRect.fromRectXY(Rect.fromLTWH(2, 2, s.width - 4, s.height - 4), 6, 6),
...
class _DeviceAlertsScreenState extends State<DeviceAlertsScreen> {
  ...
  bool _loading = true;
  bool _alertsDisabled = false; // العميل لم يفعّل أي تنبيه على الجهاز/الأجهزة
  String? _error; // فشل شبكة — يُعرض بدل «لا توجد تنبيهات» الكاذبة
  ...
  Future<void> _loadAlerts([int attempt = 0]) async {
    // الـ provider يُقرأ قبل أي await (تجنّب استخدام context بعد فجوة async)
    final provider = context.read<AppProvider>();
    if (widget.device == null && widget.allDevices == null) {
      if (mounted) setState(() => _loading = false);
      return;
    }
    ...
    String? err;
    try {
      final bool single = widget.device != null;
      final Map<String, dynamic> body = single
          ? {'traccar_id': widget.device!.traccarId, 'limit': 100}
          // كل الأجهزة في نداء واحد. كان نداءً منفصلًا لكل جهاز بالتوازي، أي ٣٢ طلبًا
          // متزامنًا من ضغطة واحدة — بطيء ويشغّل عاملًا على السيرفر لكل طلب.
          : {'traccar_id': 0, 'all': 1, 'limit': 200};
      // 15ث لا 30: السيرفر يرد في أقل من ثانية، فمحاولتان أنفع من انتظار واحد طويل
      final r = await ApiService.request('get_device_alerts', body, 15);
      if (!mounted) return;
      if (r['success'] == true) {
        final raw = (r['alerts'] as List).cast<Map<String, dynamic>>();
        final List<Map<String, dynamic>> list = [];
        if (single) {
          for (final a in raw) {
            a['_dev'] = widget.device;
          }
          list.addAll(raw);
        } else {
          // اللقطة الممرَّرة قد تكون فاضية عند الفتح البارد → نقرأ القائمة الحيّة
          var devs = widget.allDevices ?? const <DeviceModel>[];
          if (devs.isEmpty) devs = [...provider.devices, ...provider.inventory];
          final byTid = <int, DeviceModel>{
            for (final d in devs) d.traccarId: d
          };
          for (final a in raw) {
            final rv = a['deviceId'];
            final tid = rv is num ? rv.toInt() : int.tryParse('$rv') ?? -1;
            final dev = byTid[tid];
            if (dev == null) continue; // جهاز خارج نطاق الحساب
            a['_dev'] = dev;
            list.add(a);
          }
        }
        setState(() {
          _alerts = list;
          _alertsDisabled = r['alerts_disabled'] == true;
          _error = null;
          _loading = false;
        });
        return;
    ...
    if (!mounted) return;
    // فشل عابر (نت ضعيف) → محاولتان قبل إعلان الفشل
    if (attempt < 2) {
      await Future.delayed(const Duration(milliseconds: 700));
      if (!mounted) return;
      return _loadAlerts(attempt + 1);
    }
    setState(() {
      _error = err ?? tr('alerts_load_failed');
      _loading = false;
    });
  }
            ...
            : _alerts.isEmpty
                ? Center(
                    child: Column(
                        ...
                        // القائمة فاضية لأن العميل لم يفعّل أي تنبيه — نرشده بدل ترك الشاشة صامتة
                        if (_error == null && _alertsDisabled)
                          Padding(
                            padding: const EdgeInsets.fromLTRB(32, 8, 32, 0),
                            child: Text(tr('alerts_off_hint'),
                                textAlign: TextAlign.center,
                                style: const TextStyle(
                                    color: Colors.white38,
                                    fontFamily: 'Cairo',
                        ...
                        TextButton.icon(
                            ...
                            label: Text(
                                _error != null ? tr('retry') : tr('refresh'),
                                style: const TextStyle(
                                    fontFamily: 'Cairo',
                                    color: Color(0xFFF59E0B)))),
                      ]))
                : RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        final subtype =
                            (a['alarm_subtype'] ?? a['subtype'])?.toString();
                        // alarm → لون/أيقونة حسب الـ subtype (فرملة/تسارع/كهرباء...) مش العام
                        final color = type == 'alarm'
                            ? notifColorForType(type, subtype)
                            : (_colors[type] ?? const Color(0xFF8892A4));
                        final icon = type == 'alarm'
                            ? notifIconForType(type, subtype)
                            : (_icons[type] ?? Icons.notifications_outlined);
                            ...
                            child: Row(children: [
                              ...
                              Expanded(
                                  child: Column(
                                    ...
                                    Text((a['_dev'] as DeviceModel).name,
                                        style: const TextStyle(
                                            fontSize: 11,
                                            color: Colors.white38,
                                            fontFamily: 'Cairo')),
  ...
  Widget _wheel({
    ...
    return Expanded(
      child: Column(children: [
        Text(label,
            style: const TextStyle(
                color: Color(0xFF8FA8C8), fontSize: 11, fontFamily: 'Cairo')),
        const SizedBox(height: 4),
        SizedBox(
          height: 150,
          ...
          Directionality(
            ...
            child: Row(children: [
              ...
              _wheel(
                  label: tr('wp_day'),
                  count: _daysInMonth,
                  selected: (_day - 1).clamp(0, _daysInMonth - 1),
                  text: (i) => '${i + 1}',
          ...
          Row(children: [
            ...
            Expanded(
              ...
              onPressed: () {
                ...
                Navigator.pop(
                    context, DateTime(_year, _month, d, _hour, _minute));
              },
              style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFFC41E3A),
                  padding: const EdgeInsets.symmetric(vertical: 12),
...
class _AlertDetailScreenState extends State<AlertDetailScreen> {
  ...
  void initState() {
    super.initState();
    final lat = widget.alert['lat'] != null
        ? (widget.alert['lat'] as num).toDouble()
        : widget.device.lat;
    final lng = widget.alert['lng'] != null
        ? (widget.alert['lng'] as num).toDouble()
        : widget.device.lng;
              ...
              child: GoogleMap(
                ...
                markers: {
                  Marker(
                    ...
                    position: LatLng(lat, lng),
                    infoWindow: InfoWindow(
                        title: widget.label, snippet: widget.timeStr),
                  ),
                },
                myLocationButtonEnabled: false,
                zoomControlsEnabled: false,
            ...
            child: Column(children: [
              Container(
                ...
                child: Column(children: [
                  ...
                  if (widget.alert['type'] == 'deviceOverspeed' &&
                      widget.alert['speed'] != null) ...[
                    const Divider(color: Colors.white10),
                    _DetailRow(
                        icon: Icons.speed,
                        color: const Color(0xFFC41E3A),
                        label: tr('ad_recorded_speed'),
                        value:
                            '${(widget.alert['speed'] as num).round()} ${tr('unit_kmh')}'),
                    if (widget.alert['speedLimit'] != null) ...[
                      ...
                      _DetailRow(
                          ...
                          label: tr('ad_allowed'),
                          value:
                              '${(widget.alert['speedLimit'] as num).round()} ${tr('unit_kmh')}'),
                    ],
                  ],
                  if (lat != null && lng != null) ...[
                    const Divider(color: Colors.white10),
...
class _AsyncAddressText extends StatefulWidget {
  ...
  static String _clean(String a) => a
      .replaceFirst(
          RegExp(r'^[0-9A-Z]{4,8}\+[0-9A-Z]{2,3}[,،\.\s]*',
              caseSensitive: false),
          '')
      .trim();
  ...
  const _AsyncAddressText(
      {super.key, required this.lat, required this.lng, this.style});
  @override
...
class _AsyncAddressTextState extends State<_AsyncAddressText> {
  ...
  Future<void> _fetch(String k, double lat, double lng,
      [int attempt = 0]) async {
    try {
      final r = await ApiService.request(
          'reverse_geocode', {'lat': lat, 'lng': lng, 'lang': I18n.lang});
  ...
  @override
  Widget build(BuildContext context) =>
      Text(_text, style: widget.style, overflow: TextOverflow.ellipsis);
}
