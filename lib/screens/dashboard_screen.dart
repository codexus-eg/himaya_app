...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void _showAssignCardsSheet(BuildContext context) {
    ...
    final String lblTarget = isDealerActor ? 'dash_subdealer_req' : 'dash_dealer_req';
    final String lblSearch = isDealerActor ? 'dash_search_subdealer' : 'dash_search_dealer';
    final String lblPick   = isDealerActor ? 'dash_select_sub_first' : 'dash_select_dealer_first';
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Align(alignment: Alignment.centerRight, child: Text(tr(lblTarget), style: const TextStyle(fontSize: 12, color: Color(0xFF8892A4), fontFamily: 'Cairo'))),
                  const SizedBox(height: 6),
                  TextField(
                    controller: searchCtrl,
                    decoration: InputDecoration(hintText: selectedUserName ?? tr(lblSearch), prefixIcon: const Icon(Icons.search, size: 18)),
                      ...
                      child: ElevatedButton.icon(
                        ...
                        onPressed: () async {
                          if (selectedUserId == null) {
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(tr(lblPick), style: const TextStyle(fontFamily: 'Cairo')), backgroundColor: const Color(0xFFC41E3A)));
                            return;
                          }
                          ...
                          if (ctx.mounted) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content: Text(okAll
                                      ...
                                      : (failed.first['error']?.toString() ?? tr('cl_fail')),
                                  style: const TextStyle(fontFamily: 'Cairo')),
                              backgroundColor: okAll ? const Color(0xFF6BA539) : const Color(0xFFC41E3A),
                            ));
                          }
                        },
                      ),
